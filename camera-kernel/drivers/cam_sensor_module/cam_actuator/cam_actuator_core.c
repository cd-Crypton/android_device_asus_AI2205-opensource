...
//extern uint32_t vcm_offset_data[2];
...
//	int retry=3;
	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		if (rc < 0) {
			...
			del_rc = delete_request(&a_ctrl->i2c_data.init_settings);
			if (del_rc < 0) {
				CAM_ERR(CAM_ACTUATOR,
					"Fail in deleting the Init settings, skip this DAC request");
				rc = 0; //skip this requst(Do not trigger recovery retry) to avoid repeating request which causes NACK error
			}
		...
		CAM_DBG(CAM_ACTUATOR,"CHHO_V addr:0x00: reg_val %d (activated:0x66)", reg_val);

		rc = actuator_read_byte(a_ctrl, 0x1F, &reg_val0, CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_DBG(CAM_ACTUATOR,"CHHO_V addr:0x1F: reg_val %d (activated:0x3)", reg_val0);
		...
		gyro_offset = ioctl_ctrl->gyro_offset;
		CAM_DBG(CAM_ACTUATOR,"Gyro offset from hal is : 0x%x", gyro_offset);

		if(reg_val == 0x66 && reg_val0 == 0x00)
		{
			rc = vcm_g2006_initial(a_ctrl, gyro_offset);
			if (rc < 0) {
				CAM_ERR(CAM_ACTUATOR,"CHHO_V vcm_g2006_initial  Fail ", reg_val);
				goto end;
			}
			g_vcm_enabled = 1;
		}
...
extern uint8_t g_eeprom_vcm_data[EEPROM_VCM_DATA_SIZE];  //ASUS_BSP Bryant "Add for camera csi debug"

int32_t vcm_g2006_initial(struct cam_actuator_ctrl_t *a_ctrl, uint32_t gyro_offset)
{
	int rc;
    uint32_t reg_val1, reg_val2;
    uint32_t vcm_offset_data[2];


#if 0
    //-------- vcm init write on vcm.xml --------------
    ...
    static vcm_init_group_t  vcm_init_setting[] =
    {
        {0xDE, 0x00},
        {0x16, 0x44},
        {0x17, 0x03},
    ...
    int VCM_GROUP_SIZE = (sizeof(vcm_init_setting)/sizeof(vcm_init_group_t));

    CAM_DBG(CAM_ACTUATOR,"CHHO_V  vcm init TEST");

    for(i=0; i<VCM_GROUP_SIZE; i++)
    {
		rc = actuator_write_byte(a_ctrl, vcm_init_setting[i].addr, vcm_init_setting[i].data, CAMERA_SENSOR_I2C_TYPE_BYTE);
		...
		{
			mdelay(5);
			rc = actuator_read_byte(a_ctrl, vcm_init_setting[i].addr, &reg_val, CAMERA_SENSOR_I2C_TYPE_BYTE);
			msleep(1);
			pr_err("CHHO_V setting[%d].addr:0x%x, data:0x%x, read data:0x%x", i, vcm_init_setting[i].addr, vcm_init_setting[i].data, reg_val);
		}
    }
#endif


    //eeprom copy to vcm debug
	CAM_DBG(CAM_ACTUATOR,"CHHO_V   eeprom copy to vcm debug");
	rc = actuator_write_seq_bytes(a_ctrl, 0x06, g_eeprom_vcm_data, EEPROM_VCM_DATA_SIZE, CAMERA_SENSOR_I2C_TYPE_BYTE);

#if 0
	//eeprom copy to vcm debug
	for( i = 0 ; i < EEPROM_VCM_DATA_SIZE; i++)
	{
		rc = actuator_read_byte(a_ctrl, (0x06+i), &reg_val, CAMERA_SENSOR_I2C_TYPE_BYTE);
		pr_err("CHHO_V eeprom addr:0x%x, data:0x%x, vcm adder:0x%x read data:0x%x", i+0x1C59, g_eeprom_vcm_data[i], (0x06+i), reg_val);
	}

#endif

	CAM_DBG(CAM_ACTUATOR,"CHHO_V Setting Gyro offset debug");

	vcm_offset_data[0] = gyro_offset >> 8;
	vcm_offset_data[1] = gyro_offset & 0x00FF;
	rc = actuator_write_byte(a_ctrl, 0x95, vcm_offset_data[0], CAMERA_SENSOR_I2C_TYPE_BYTE);
	rc = actuator_write_byte(a_ctrl, 0x96, vcm_offset_data[1], CAMERA_SENSOR_I2C_TYPE_BYTE);
	mdelay(2);
	rc = actuator_read_byte(a_ctrl, 0x95, &reg_val1, CAMERA_SENSOR_I2C_TYPE_BYTE);
	CAM_DBG(CAM_ACTUATOR,"CHHO_V  vcm_offset_data addr:0x95   data: 0x%x", reg_val1);
	rc = actuator_read_byte(a_ctrl, 0x96, &reg_val2, CAMERA_SENSOR_I2C_TYPE_BYTE);
	CAM_DBG(CAM_ACTUATOR,"CHHO_V  vcm_offset_data addr:0x96   data: 0x%x", reg_val2);

    CAM_DBG(CAM_ACTUATOR,"CHHO_V  VCM start");
    rc = actuator_write_byte(a_ctrl, 0x02, 0x8A, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x1C, 0x80, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x99, 0x00, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x9A, 0x00, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0xDB, 0x1F, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0xDC, 0xFF, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0xDD, 0x1F, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0xDF, 0xFF, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x1F, 0x01, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x03, 0x01, CAMERA_SENSOR_I2C_TYPE_BYTE);
    rc = actuator_write_byte(a_ctrl, 0x04, 0x40, CAMERA_SENSOR_I2C_TYPE_BYTE);
    mdelay(20);
    rc = actuator_write_byte(a_ctrl, 0x1F, 0x03, CAMERA_SENSOR_I2C_TYPE_BYTE);
