...
static int32_t cam_eeprom_pkt_parse(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		...
		CAM_INFO(CAM_EEPROM, "[eeprom_debug] eeprom soc index  %d   camera_id %d", e_ctrl->soc_info.index, camera_id);
		...
		if (e_ctrl->soc_info.index < MAX_LEGAL_EEPROM_NUMBER) //normal read eeprom
		{
			...
			CAM_INFO(CAM_EEPROM, "[eeprom_debug] cam_eeprom_power_up success");
...
/*
			for(i = 0x00 ; i < e_ctrl->cal_data.num_data; i++)
			{
				...
				CAM_ERR(CAM_EEPROM,"[eeprom_debug] CHHO  eeprom soc index  %d  EEPROM DATA i= %04x data = %02x  ", e_ctrl->soc_info.index, i, e_ctrl->cal_data.mapdata[i]);
			}
*/
			CAM_INFO(CAM_EEPROM,"[eeprom_debug] EEPROM INIT done");
		...
		}else //it is use fake eeprom and read otp data to eeprom data.
		{
			...
			*********************************************/
			...
			CAM_INFO(CAM_EEPROM, "[eeprom_debug] OTP only, Skip eeprom power up/down and read eeprom");
