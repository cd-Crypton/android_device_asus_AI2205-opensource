...
void set_ov32c4c_main_mode(struct cam_sensor_ctrl_t *ctrl); //ASUS_BSP Neo: set ov32c4c to main sensor mode
...
//Camera BSP bypass ov13b2A chip id in ER1 +++
	...
	if (!of_property_read_u32_array(root, "qcom,board-id", board_id, 2)) {
		hwid = board_id[0];
		CAM_DBG(CAM_SENSOR, "Get hwid = %d", hwid);
	}
#endif
//Camera BSP bypass ov13b2A chip id in ER1 ---
...
#ifdef ASUS_AI2205_PROJECT
	//Camera BSP bypass ov13b2A chip id in ER1
	if(chipid == 0xD43 && hwid == AI2205_ER1) return rc;
#endif
...
//ASUS_BSP Neo: set ov32c4c to main sensor mode +++
void set_ov32c4c_main_mode(struct cam_sensor_ctrl_t *ctrl)
{
	int rc = 0;
	int always_on_slave_addr = 0x54;
	int main_sensor_sid = 0;

	struct cam_sensor_i2c_reg_setting write_setting;
	struct cam_sensor_i2c_reg_array data;

	if(ctrl->sensordata->slave_info.sensor_id == 0x3243) //ov32c4c sensor_id
	{
		CAM_INFO(CAM_SENSOR, "ov32c4c always-on slave address: 0x%x", always_on_slave_addr);
		main_sensor_sid = ctrl->io_master_info.cci_client->sid;
		ctrl->io_master_info.cci_client->sid = (always_on_slave_addr>>1); //ov32c4c always-on mode sid

		data.reg_addr = 0x1001;
		data.reg_data = 4; //normal mode
		data.delay = 0;
		...
		if(rc < 0)
		{
			CAM_ERR(CAM_SENSOR, "sid:0x%x write 0x%x to reg 0x%x failed! rc = %d",
				ctrl->io_master_info.cci_client->sid, data.reg_data, data.reg_addr, rc);
		}

		ctrl->io_master_info.cci_client->sid = main_sensor_sid;
	}
}
//ASUS_BSP Neo: set ov32c4c to main sensor mode ---
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		CAM_INFO(CAM_SENSOR, "CHHO Power up Sensor %s cam_sensor_power_up", s_ctrl->sensor_name);
		...
		set_ov32c4c_main_mode(s_ctrl); //ASUS_BSP Neo: set ov32c4c to main sensor mode
		/* Match sensor ID */
