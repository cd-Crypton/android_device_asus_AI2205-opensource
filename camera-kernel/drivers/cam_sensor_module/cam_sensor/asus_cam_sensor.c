...
#define CSIPHY_MAX (4)//CSIPHY 0 =imx766 1 =ov13b 2=ov885 3 =ov32c //ASUS_BSP "Add for camera csi phy id debug count"


static uint8_t  g_sensor_init_state[MAX_CAMERA_ID+1]={0};
static uint16_t g_sensor_id[MAX_CAMERA_ID+1]={0};

uint8_t  g_eeprom_vcm_data[EEPROM_VCM_DATA_SIZE]={0};
...
uint8_t g_cam_csiphy[4]={0};//CSIPHY 0 =imx766 1 =ov13b 2=ov885 3 =ov32c //ASUS_BSP "Add for camera csi phy id debug count"
...
static sensor_define_t project_camera[MAX_CAMERA_ID+1] = {
	...
	{SENSOR_ID_OV32C4C, DIRECTION_FRONT},//ASUS_BSP Neo add for cam 1
...
//============ dualcam_cail +++ ==============
#define DUAL_CALI_START 0x303A //0x821
#define DUAL_CALI_END	0x3839 //0x1020
...
{
	int i;
	int report_eeprom_dual=0;
	eeprom_info_t * eeprom_info = (eeprom_info_t *)buf->private;

	if(eeprom_info == NULL || eeprom_info->mapdata == NULL || eeprom_info->num_data < dual_cali_end+1)
	{
		pr_err("eeprom is NULL");
		return 0;
	}
	...
	if(report_eeprom_dual >= 2000){
		buf->count = 0;
		pr_err("eeprom dualcam part is invalid\n");
	}
	return 0;
}
...
static struct proc_ops dual_cali_fops = {
	//.owner = THIS_MODULE,
	.proc_open = dual_cali_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};

static void dual_cali_dump_remove() {
	remove_proc_entry(PROC_ARCSOFT_CALI, NULL);
}
//create_proc_file for Arcsoft_calibration (for camera alignment: e.g. Bokeh, SAT)
//only for camera0
static void dual_cali_dump_create()
{
	proc_create_data(PROC_ARCSOFT_CALI, 0444, NULL, &dual_cali_fops, &g_eeprom_info[0]); //IMX766_R is index 0
}
//============ dualcam_cail --- ==============
...
static int camera_vcm_data_read(struct seq_file *buf, void *v)
{
	...
	else
	{
		...
		for( i = 0x1C59 ; i <= 0x1D49; i++)
		{
			seq_printf(buf, "0x%04X 0x%02X   g_eeprom_vcm_data 0x%02X   \n",i,eeprom->mapdata[i], g_eeprom_vcm_data[i-0x1C59]);
		}
	}
	return 0;
}
...
void save_eeprom_vcm_data(uint8_t moduleGroupIndex)
{
	/*
	int i=0;
	for( i = 0x1C59 ; i <= 0x1D49; i++)
	...
	*/
	memcpy(g_eeprom_vcm_data, g_eeprom_info[moduleGroupIndex].mapdata + 0x1C59, EEPROM_VCM_DATA_SIZE);
}
...
void eeprom_dump_create(struct cam_eeprom_ctrl_t * e_ctrl, uint8_t moduleGroupIndex)
{
	...
	if(g_eeprom_ctrl[moduleGroupIndex] == NULL)//first time
	{
		...
		if (e_ctrl->soc_info.index == 0)
		{
			...
			//save eeprom vcm data
			save_eeprom_vcm_data(moduleGroupIndex);
		}
	}
	else
	{
...
static int  cam_csi_phy_check_read(struct seq_file *buf, void *v)
{
	ssize_t ret = 0;
	int i=0;

	for(i=0;i<CSIPHY_MAX;i++){

		pr_err("camera csi phy[%] check count %6d\n",i, g_cam_csiphy[i]);
		seq_printf(buf, "%6d\n",g_cam_csiphy[i]);
...
static int cam_csi_phy_check_open(struct inode *inode, struct	 file *file)
{
	return single_open(file, cam_csi_phy_check_read, NULL);
}
...
static struct proc_ops cam_csi_phy_check_count_fops = {
	.proc_open  = cam_csi_phy_check_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static void cam_csi_phy_debug_count_create(void)
{
	create_proc_file(PROC_CSI_PHY_CHECK_COUNT, &cam_csi_phy_check_count_fops, NULL);
}
...
void asus_cam_sensor_init(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if(!g_sensor_init_state[s_ctrl->id])
	{
		...
		if(s_ctrl->id == CAMERA_0)
		{
            ...
            cam_csi_phy_debug_count_create();
		}
