...
extern uint8_t  g_cam_csiphy[4];  //ASUS_BSP "Add for camera csi phy id debug count"
#endif
...
int32_t cam_csiphy_common_status_reg_dump(struct csiphy_device *csiphy_dev)
{
	...
	if (phy_base != NULL) {
        ...
        #if defined ASUS_AI2205_PROJECT
        g_cam_csiphy[csiphy_dev->soc_info.index]++;//ASUS_BSP "Add for camera csi phy id debug count
        #endif
	} else {
		rc = -EINVAL;
