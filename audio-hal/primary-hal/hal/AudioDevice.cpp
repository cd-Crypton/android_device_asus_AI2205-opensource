...
static bool hdr_set_parameters(std::shared_ptr<AudioDevice> adev,
    ...
    ret = str_parms_get_int(parms, AUDIO_PARAMETER_KEY_HDR_ROTATION, &val);
    if (ret >= 0) {
        adev->inverted = (val == ROTATION_180|| val == ROTATION_270)? true : false;
        adev->orientation_landscape = (val == ROTATION_0 || val == ROTATION_180)? true : false;
        changes = true;
...
// ASUS_BSP for SMMI input/output select +++
    ...
    int mic_selected = 0;
// ASUS_BSP for SMMI input/output select ---
// ASUS_BSP for game with 3pole use speaker-mic +++
...
// ASUS_BSP for SMMI input select +++
            mic_selected = adev_->smmi_tool_mic_test;
            AHAL_INFO("mic_selected %d", mic_selected);
            if (mic_selected != 0) {
                switch (mic_selected) {
                    case 1:
                        AHAL_INFO("HandsetMic device - mic1");
                        device_types.insert(AUDIO_DEVICE_IN_BUILTIN_MIC);
                        break;
                    case 2:
                        AHAL_INFO("HandsetMic device - mic2");
                        device_types.insert(AUDIO_DEVICE_IN_BUILTIN_MIC);
                        break;
                    case 3:
                        ...
                        break;
                    case 4:
                        AHAL_INFO("HandsetMic device - mic3");
                        device_types.insert(AUDIO_DEVICE_IN_BUILTIN_MIC);
                        break;
                }
// ASUS_BSP remove Game mode mic for Handsfree of voice call/VoIP call/Game VoIP call in sync
                ...
                AHAL_INFO("SpeakerMic device for Game mode Currently, maybe others in the future");
                device_types.insert(AUDIO_DEVICE_IN_BACK_MIC);
            } else if ((wired_3pole || usb_3pole) &&
                       adev_->game_mode_enabled &&
                       (sources[0].ext.device.type == AUDIO_DEVICE_IN_BUILTIN_MIC)) {
                // for VoIP or game with 3-pole headphones uses speaker-mic
                AHAL_INFO("SpeakerMic device for Game mode");
                device_types.insert(AUDIO_DEVICE_IN_BACK_MIC);
            } else
            ...
            if ((sinks.size() == 1) && (output_selected != 0)) {
                switch (output_selected) {
                    ...
                    case 4:
                        ...
                        break;
                    case 9:
                        AHAL_INFO("Inbox Speaker device");
                        device_types.insert(AUDIO_DEVICE_OUT_SPEAKER);
                        break;
                }
            } else
...
//Jessy +++ ASUS game voip feature
        pal_param_voip_state param_voip_st;
        if(mode ==AUDIO_MODE_IN_COMMUNICATION) {
            param_voip_st.voip_state=true;
        } else {
            param_voip_st.voip_state=false;
        }
        ret = pal_set_param( PAL_PARAM_ID_VOIP_STATE, (void*)&param_voip_st, sizeof(pal_param_voip_state_t));
//Jessy ---
    AHAL_DBG("Exit ret: %d", ret);
    return ret;
}
...
//Jessy +++ outdoor mode
    ...
    if (ret >= 0) {
        ...
        if (val && !(adev_->active_stream_state & OUTDOOR_RING)) {
            AHAL_INFO("%s: skip set ring outdoor when stream ring is not playing", __func__);
            skipSetOutdoor = true;
        }
    ...
    if (ret >= 0) {
        ...
        if ((adev_->active_stream_state & OUTDOOR_MUSIC) && (val)) {
            AHAL_INFO("%s: skip set notification outdoor during music playback ", __func__);
            skipSetOutdoor = true;
        }
...
//Jessy +++ ASUS game voip feature
            pal_param_game_state param_game_st;
            if(adev_->game_mode_enabled) {
                param_game_st.game_state=true;
            } else {
                param_game_st.game_state=false;
            }
            ret = pal_set_param( PAL_PARAM_ID_GAME_STATE, (void*)&param_game_st, sizeof(pal_param_game_state_t));
//Jessy ---
...
//Jessy +++
    ret = str_parms_get_str(parms, "rog_afk", value, sizeof(value));
    if (ret >= 0) {
        ALOGD("rog_afk %s", value);
        if ((!strcmp(value, "on") && !adev_->rog_afk_enabled) ||
                (!strcmp(value, "off") && adev_->rog_afk_enabled)) {
            adev_->rog_afk_enabled = !adev_->rog_afk_enabled;
            ALOGD("Toggle afk mode");
        }
    }
//Jessy ---
...
//Jessy +++
    ret = str_parms_get_str(query, "rog_afk", value, sizeof(value));
    if (ret >= 0) {
        bool rog_afk_enabled = false;
        rog_afk_enabled = adev_->rog_afk_enabled;

        ALOGD("getting afk mode (%d)", rog_afk_enabled);

        if (rog_afk_enabled) {
            str_parms_add_str(reply, "rog_afk", "on");
            goto exit;
        } else {
            str_parms_add_str(reply, "rog_afk", "off");
            goto exit;
        }
    }
//Jessy ---
...
int AudioDevice::GetPalDeviceIds(const std::set<audio_devices_t>& hal_device_ids,
    ...
    AHAL_VERBOSE("haldeviceIds: %zu", hal_device_ids.size());
    ...
    for(auto hal_device_id : hal_device_ids) {
        ...
        if (it != android_device_map_.end() &&
            ...
            AHAL_VERBOSE("Found haldeviceId: %x and PAL Device ID %d",
                    it->first, it->second);
...
error:
    AHAL_VERBOSE("devices allocated %zu, pal device ids before returning %d",
...
#define CRUS_WOOFER_DSP_FIRMWARE_MIXER         "WOOFER DSP1 Firmware"
#define CRUS_WOOFER_DSP_PRELOAD_MIXER          "WOOFER DSP1 Preload Switch"
#define CRUS_WOOFER_DSP_BOOTED_MIXER           "WOOFER DSP1 Boot Switch"
#define CRUS_WOOFER_DSP_CAL_R_MIXER            "WOOFER DSP1 Protection cd CAL_R"
#define CRUS_WOOFER_DSP_CAL_STATUS_MIXER       "WOOFER DSP1 Protection cd CAL_STATUS"
#define CRUS_WOOFER_DSP_CAL_CHECKSUM_MIXER     "WOOFER DSP1 Protection cd CAL_CHECKSUM"
#define CRUS_WOOFER_DSP_PARAMS_MIXER           "WOOFER DSP1 Protection f207 MDSYNC_WAITING"
#define CRUS_WOOFER_CALI_RESISTANCE_MIXER      "WOOFER DSP Set CAL_R"
...
#define WOOFER_CAL_VAL_MIN 7728
#define WOOFER_CAL_VAL_MAX 10454

#define WOOFER_INBOX_CAL_VAL_MIN 4755
#define WOOFER_INBOX_CAL_VAL_MAX 6433
...
#define CRUS_WOOFER_LOCAL_CAL_R_FILE "/vendor/factory/audio/woofer_cal_val"
...
static int adev_crus_woofer_smartpa_init(void)
{
	static bool crus_woofer_smartpa_inited = false;
	struct mixer *mixer = NULL;
	struct mixer_ctl *ctl = NULL;
	char cali_file[256] = {0};
	int woofer_cal_r = 4755; // For example, speaker calibration resistance=8900, Z_TO_OHM(8900)=6.36 ohm
	int retry;
	int ret;

	ALOGD("%s: enter", __func__);

	if (!crus_woofer_smartpa_inited) {
		/* First of all, check if smartpa driver is ready in case of working as module */
		for (retry = 300; retry; retry--) {
			if (mixer) {
				mixer_close(mixer);
			...
			// Check Woofer
			ctl = mixer_get_ctl_by_name(mixer, CRUS_WOOFER_DSP_FIRMWARE_MIXER);
			if (!ctl) {
				ALOGE("%s: Could not get ctl for mixer cmd - %s(%d)", __func__, CRUS_WOOFER_DSP_FIRMWARE_MIXER, retry);
				continue;
			}
			ALOGE("%s: Get mixer cmd - %s(%d)", __func__, CRUS_WOOFER_DSP_FIRMWARE_MIXER, retry);
		...
		/* Driver is not ready yet? */
		if (!retry) {
			ALOGE("%s: Woofer SmartPA driver is not ready yet!", __func__);
			return -EINVAL;
		}

		/* Next, restore smartpa calibration data from platform non-volatile memory */
		sprintf(cali_file, "%s", CRUS_WOOFER_LOCAL_CAL_R_FILE);
		if (readCalValue(cali_file, &woofer_cal_r)) {
			ALOGE("%s: read woofer_cal_r %s failed", __func__, cali_file);
			woofer_cal_r = 0;
		}
		ALOGD("%s: woofer_cal_r:%d", __func__, woofer_cal_r);

		/* Update woofer calibration resistance and check */
		if (woofer_cal_r == 0 )	{
			ALOGD("%s: woofer_cal_r:%d value not normal ignore AMP calibration value appliy", __func__, woofer_cal_r);
		} else if (woofer_cal_r < WOOFER_INBOX_CAL_VAL_MIN || woofer_cal_r > WOOFER_INBOX_CAL_VAL_MAX) {
			ALOGD("%s: woofer_cal_r:%d value out of range ignore AMP calibration value appliy", __func__, woofer_cal_r);
		} else	{
			ctl = mixer_get_ctl_by_name(mixer, CRUS_WOOFER_CALI_RESISTANCE_MIXER);
			if (!ctl)	{
				ALOGE("%s: Could not get ctl for mixer cmd - %s", __func__, CRUS_WOOFER_CALI_RESISTANCE_MIXER);
			} else	{
				ret = mixer_ctl_set_value(ctl, 0, woofer_cal_r);
				if (ret) {
					ALOGE("%s: Failed to set ctl - %s", __func__, CRUS_WOOFER_CALI_RESISTANCE_MIXER);
				}
			}
			ALOGE("%s: Finish mixer cmd - %s", __func__, CRUS_WOOFER_CALI_RESISTANCE_MIXER);
		}


		/* Now start to preload woofer firmware */
		// Preload Woofer firmware
		ctl = mixer_get_ctl_by_name(mixer, CRUS_WOOFER_DSP_FIRMWARE_MIXER);
		if (!ctl)	{
			ALOGE("%s: Could not get ctl for mixer cmd - %s", __func__, CRUS_WOOFER_DSP_FIRMWARE_MIXER);
		} else	{
			ret = mixer_ctl_set_enum_by_string(ctl, CRUS_DSP_FIRMWARE_STRING);
			if (ret) {
				ALOGE("%s: Failed to set ctl - %s", __func__, CRUS_DSP_FIRMWARE_STRING);
		...
		ctl = mixer_get_ctl_by_name(mixer, CRUS_WOOFER_DSP_PRELOAD_MIXER);
		if (!ctl)	{
			ALOGE("%s: Could not get ctl for mixer cmd - %s", __func__, CRUS_WOOFER_DSP_PRELOAD_MIXER);
		} else {
			ret = mixer_ctl_set_value(ctl, 0, 1);
			if (ret)	{
				ALOGE("%s: Failed to set ctl - %s", __func__, CRUS_WOOFER_DSP_PRELOAD_MIXER);
			}
			ALOGE("%s: Finish mixer cmd - %s", __func__, CRUS_WOOFER_DSP_PRELOAD_MIXER);
		}

		ctl = mixer_get_ctl_by_name(mixer, CRUS_WOOFER_DSP_BOOTED_MIXER);
		if (!ctl)	{
			ALOGE("%s: Could not get ctl for mixer cmd - %s", __func__, CRUS_WOOFER_DSP_BOOTED_MIXER);
		} else {
			ret = mixer_ctl_set_value(ctl, 0, 1);
			if (ret)	{
				ALOGE("%s: Failed to set ctl - %s", __func__, CRUS_WOOFER_DSP_BOOTED_MIXER);
			}
			ALOGE("%s: Finish mixer cmd - %s", __func__, CRUS_WOOFER_DSP_BOOTED_MIXER);
		}

		crus_woofer_smartpa_inited = true;
		property_set("vendor.audio.crus.smartpa.inited", "true");
	}
