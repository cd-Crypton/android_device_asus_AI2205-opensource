...
#define USB_FULL_PROP "vendor.asus.usb.fullspeed"
#define USB_SPEED_PATH "/sys/bus/platform/devices/a600000.ssusb/speed"
...
Status getPortStatusHelper(hidl_vec<PortStatus> *currentPortStatus_1_2,
  ...
  if (result == Status::SUCCESS) {
    ...
    for (std::pair<std::string, bool> port : names) {
      ...
      if (port.first == "port1")
        (*currentPortStatus_1_2)[i].status_1_1.status.canChangeDataRole &= !(usb->mPowerOpMode == "default");
      ...
      if (port.first == "port1")
        (*currentPortStatus_1_2)[i].status_1_1.status.canChangePowerRole &= !(usb->mPowerOpMode == "default");
...
static void handle_typec_uevent(Usb *usb, const char *msg)
{
  ...
  if (!readFile("/sys/class/typec/port1/power_operation_mode", &power_operation_mode)) {
...
static void uevent_event(uint32_t /*epevents*/, struct data *payload) {
  ...
  std::string fullspeed = GetProperty(USB_FULL_PROP, "");
  ...
  for (i=0;*p!='\0' && i<n;p+=strlen(p)+1, i++) {
     if(strstr(p,"F_ACC_SPEED=FULL")) {
         ALOGE("ERROR:restarting in full speed");
	 if (strstr(fullspeed.c_str(), "1")) {
         	readFile(USB_SPEED_PATH, &priv_speed);
         	writeFile(USB_SPEED_PATH, "full");
	 	ALOGE("priv speed is %s\n", priv_speed.c_str());
	 }
     } else if (strstr(p,"F_ACC_SPEED=RESET")) {
         ALOGE("resetting speed to %s\n", priv_speed.c_str());
         if(strstr(priv_speed.c_str(),"super-speed-plus")) {
	     ALOGE("reset speed to ssp");
             writeFile(USB_SPEED_PATH, "ssp");
	 } else if (strstr(priv_speed.c_str(),"super-speed")) {
  	     ALOGE("reset speed to super");
             writeFile(USB_SPEED_PATH, "super");
	 } else if (strstr(priv_speed.c_str(),"high-speed")) {
	     ALOGE("reset speed to high");
             writeFile(USB_SPEED_PATH, "high");
	 } else if (strstr(priv_speed.c_str(),"full-speed")){
             ALOGE("reset speed to full");
             writeFile(USB_SPEED_PATH, "full");
	 }
         priv_speed = '\0';
     }
