/*
 * Copyright (c) 2022, ASUS. All rights reserved.
 */
...
enum power_callback_state_type power_callback_state = POWER_CALLBACK_STATE_OUT_RESUME;
enum power_callback_state_type power_state_commit_work = POWER_CALLBACK_STATE_OUT_RESUME;
enum power_callback_state_type power_state_when_commit = POWER_CALLBACK_STATE_OUT_RESUME;
...
static void set_tcon_cmd(char *cmd, short len)
{
	...
	struct mipi_dsi_msg tcon_cmd = {0, 0x15, 0, len, cmd, 0, NULL};
...
static void get_tcon_cmd(char cmd, int rlen)
{
	...
	struct dsi_cmd_desc cmds;
	struct mipi_dsi_msg tcon_cmd = {0, 0x06, 0, sizeof(cmd), NULL, rlen, NULL};
	const struct mipi_dsi_host_ops *ops = g_display->panel->host->ops;
...
static void get_reg_cmd(char cmd, int len, int head)
{
	...
	struct dsi_cmd_desc cmds;
	struct mipi_dsi_msg tcon_cmd = {0, 0x06, 0, sizeof(cmd), NULL, len, NULL};
	const struct mipi_dsi_host_ops *ops = g_display->panel->host->ops;
...
static int dsi_ai2205_tx_cmd_set(struct dsi_panel *panel,
				enum dsi_cmd_set_type type)
{
	...
	if (!panel->post_on) {
		DSI_LOG("post enable first, type = %d", type);
		panel->post_on = true;
		rc = dsi_panel_asus_post_enable(panel);
	}
...
static int dsi_ai2205_set_hbm(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_HBM_ON);
		if (rc)
			DSI_LOG("[%s] failed to send DSI_CMD_SET_HBM_ON cmd, rc=%d\n",
				   panel->name, rc);
	} else {
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_HBM_OFF);
		if (rc)
			DSI_LOG("[%s] failed to send DSI_CMD_SET_HBM_ON cmd, rc=%d\n",
				   panel->name, rc);
	}
...
{
	if (mode == 0 && g_display->panel->panel_fod_hbm_mode) {
		DSI_LOG("hbm on by fod now.\n");
		return;
	} else if (mode == g_display->panel->panel_hbm_mode) {
		DSI_LOG("hbm mode same.\n");
	}

	g_display->panel->panel_hbm_mode = mode;
	DSI_LOG("hbm mode is (%d)\n", g_display->panel->panel_hbm_mode);
	dsi_ai2205_set_hbm(g_display->panel, mode);
}
...
static void display_exit_idle_mode()
{
	...
	rc = dsi_ai2205_tx_cmd_set(g_display->panel, DSI_CMD_SET_NOLP);
	if (rc)
...
static int dsi_ai2205_set_fod_hbm(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	...
	if (enable) {
		...
		if (panel->fod_in_doze) {
			...
			rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_LP1);
			if (rc)
				...
				rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_TIMING_SWITCH);
				if (rc)
			...
			rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_FOD_HBM_ON);
			if (rc)
				DSI_LOG("[%s] failed to send DSI_CMD_SET_FOD_HBM_ON cmd, rc=%d\n",
					   panel->name, rc);
		}
	} else {
		dsi_ai2205_restore_backlight();

		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_FOD_HBM_OFF);
		if (rc)
...
static void display_set_fod_hbm_mode(int mode)
{
	...
	dsi_ai2205_set_fod_hbm(g_display->panel, mode);
}
...
static void display_set_fod_hbm(void)
{
	...
	dsi_ai2205_set_fod_hbm(g_display->panel, g_display->panel->allow_panel_fod_hbm);
	ai2205_drm_notify(ASUS_NOTIFY_GHBM_ON_REQ, g_display->panel->allow_panel_fod_hbm);
	DSI_LOG("FOD: global hbm <fod> (%d) --- \n", g_display->panel->allow_panel_fod_hbm);
	...
	if (g_display->panel->allow_panel_fod_hbm == 0) {
		...
		if (g_display->panel->panel_hbm_mode) dsi_ai2205_set_hbm(g_display->panel, 1);
	}
}
...
void dsi_ai2205_set_dimming_smooth(struct dsi_panel *panel, u32 backlight)
{
	...
	if (panel->panel_bl_count == 1 && g_display->panel->panel_hbm_mode != 3) {
		DSI_LOG("restore dimming smooth\n");
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_DIMMING_SMOOTH);
		if (rc)
...
void dsi_ai2205_frame_commit_cnt(struct drm_crtc *crtc)
{
	...
	if(g_display->panel->aod_delay && g_display->panel->aod_state) {
		...
		if(aod_delay_frames >= 5 && !(g_display->panel->allow_panel_fod_hbm || g_display->panel->allow_fod_hbm_process)) {
			dsi_ai2205_need_aod_reset(g_display->panel);
			aod_delay_frames = 0;
		}
	}
	...
	if (display_commit_cnt == 0 && g_display->panel->aod_state) {
		if (g_display->panel->aod_mode == -1) {
			...
			if (g_display->panel->panel_last_backlight > 15) {
				...
				dsi_panel_set_backlight(g_display->panel, 89);
			} else {
				...
				size_t count, loff_t *ppos)
{
	...
	ssize_t ret = 0;
	char *buff;
	char cmd1[3] = {0xfc, 0x5a, 0x5a};
	char cmd2[2] = {0xb0, 0x60};
	char cmd3[3] = {0xfc, 0xa5, 0xa5};

	buff = kzalloc(256, GFP_KERNEL);
	if (!buff)
		return -ENOMEM;

	mutex_lock(&g_display->id_lock);
	if (strlen(g_reg_read) == 0) {
		memset(g_reg_read, 0, REG_BUF_SIZE*sizeof(char));
		set_tcon_cmd(cmd1, 3);
		set_tcon_cmd(cmd2, 2);
		get_reg_cmd(0xd1, 5, 0);
		set_tcon_cmd(cmd3, 3);
	...
	ret = simple_read_from_buffer(buf, count, ppos, buff, len);
	mutex_unlock(&g_display->id_lock);
...
// commit_wakelock_ops() - Debug use, can disable wakelock when commit
int commit_wakelock = 1;
static ssize_t commit_wakelock_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	char messages[256];
	memset(messages, 0, sizeof(messages));
	...
	if (strncmp(messages, "0", 1) == 0) {
		...
		DSI_LOG("don't match any commit_wakelock.\n");
	}

	return len;
}

static ssize_t commit_wakelock_read(struct file *file, char __user *buf,
							 size_t count, loff_t *ppos)
{
	int len = 0;
	ssize_t ret = 0;
...
static struct proc_ops commit_wakelock_ops = {
	.proc_write = commit_wakelock_write,
	.proc_read  = commit_wakelock_read,
};

// send dimming command to panel
static int dsi_ai2205_set_dimming(struct dsi_panel *panel, bool enable)
{
	int rc = 0;
	...
	if (enable)
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_DIMMING_SPEED_1);
	else
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_DIMMING_SPEED_20);
...
static ssize_t dimming_speed_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	...
	if (g_display->panel->panel_is_on
		...
		if (strncmp(messages, "1", 1) == 0) {
			rc = dsi_ai2205_set_dimming(g_display->panel, true);
		} else if (strncmp(messages, "20", 20) == 0) {
			// restore by dsi_ai2205_set_dimming_smooth
			//rc = dsi_ai2205_set_dimming(g_display->panel, false);
		} else {
...
void ai2205_set_err_fg_irq_state(bool state) {
	if(g_display->panel->panel_is_on || !state) {
		...
		if(state) {
			...
			ai2205_set_err_fg_pending(true);
		}
	}
}
...
static ssize_t video_mode_show(struct class *class,
					struct class_attribute *attr,
					char *buf)
{
	return sprintf(buf, "%d\n", g_video);
...
static ssize_t video_mode_store(struct class *class,
					struct class_attribute *attr,
					const char *buf, size_t count)
{
	if (!count)
...
static CLASS_ATTR_RW(video_mode);
...
static void dsi_create_drm_class_obj(void)
{
	...
	err = class_create_file(drm_dev.class, &class_attr_video_mode);
	if (err) {
		pr_err("[Display] Fail to create video_mode file node\n");
	}
...
/**
 * dsi_ai2205_display_init() - to initial display parameters
...
{
	DSI_LOG("AI2205 Display Init");

	dsi_create_drm_class_obj();

	g_display = display;
	dsi_ai2205_parse_panel_vendor_id(g_display->panel);
	...
	proc_create(COMMIT_WAKELOCK, 0666, NULL, &commit_wakelock_ops);
}
...
void dsi_ai2205_set_panel_is_on(bool on)
{
	...
	if (on == false) {
		...
		ai2205_drm_notify(ASUS_NOTIFY_SPOT_READY, 0);
		ai2205_drm_notify(ASUS_NOTIFY_GHBM_ON_READY, 0);
		ai2205_drm_notify(ASUS_NOTIFY_GHBM_ON_REQ, 0);
		g_display->panel->is_gamma_change = false;
		g_display->panel->aod_delay = false;
		display_panel_clear_dc();
		g_display->panel->csc_mode = 0;
...
static void dsi_ai2205_aod_backlight(struct dsi_panel *panel)
{
	int rc = 0;
	if (!panel) {
		DSI_LOG("invalid params\n");
	...
	if (panel->aod_state && !(panel->allow_panel_fod_hbm || panel->allow_fod_hbm_process)) {
		// for bl= 61 & bl=89 & bl=9
		if (panel->panel_last_backlight == 61 || panel->panel_last_backlight == 89 || panel->panel_last_backlight == 9) {
			if ((panel->panel_last_backlight == 61 || panel->panel_last_backlight == 89) && panel->panel_last_backlight != panel->panel_aod_last_bl) {
				DSI_LOG("set AOD HIGH command\n");
				rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_AOD_HIGH);
				...
				rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_AOD_LOW);
				panel->panel_aod_last_bl = panel->panel_last_backlight;
				panel->aod_mode = 0;
			}
		} else { // to prevent display off
			DSI_LOG("set AOD Other command\n");
			rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_AOD_OTHER);
		}
...
void dsi_ai2205_record_backlight(u32 bl_lvl)
{
	...
	if ((g_display->panel->power_mode == 1) ||
		...
		dsi_ai2205_aod_backlight(g_display->panel);
	}
}
...
void dsi_ai2205_restore_backlight(void)
{
	...
	if (g_display->panel->panel_last_backlight == 0)
		return;
...
{
	if (g_display->panel->post_on && g_display->panel->allow_fod_hbm_process && !strcmp(crtc->name, "crtc-0")) {
		...
		if (g_display->panel->allow_panel_fod_hbm == 1) {
			...
			ai2205_drm_notify(ASUS_NOTIFY_GHBM_ON_READY, 1);
...
static void dsi_ai2205_set_notify_spot_ready(int type)
{
	uint32_t delta_us = 0;
	uint32_t frame_us = 0;
	uint32_t delay = 0;
	...
	if (is_ready && !fod_spot_ui_ready) {
		...
		// TT#369534 workaround for preventing ui ready notify too early
		DSI_LOG("ui ready notify after %d (us)", frame_us);
		usleep_range(frame_us, frame_us);
		ai2205_drm_notify(ASUS_NOTIFY_SPOT_READY, 1);
	}
}
...
void ai2205_set_notify_spot_ready(struct drm_crtc *crtc) {
	...
	dsi_ai2205_set_notify_spot_ready(2);
}
...
void ai2205_atomic_set_spot_status(int type)
{
	...
	if (type == 0) {
		...
		if (g_display->panel->fod_in_doze) {
			rc = dsi_ai2205_tx_cmd_set(g_display->panel, DSI_CMD_SET_POST_FOD_HBM_ON);
			if (rc) {
	...
	dsi_ai2205_set_notify_spot_ready(1);
}
...
void dsi_ai2205_need_aod_reset(struct dsi_panel *panel)
{
	...
	if(panel->panel_last_backlight == 9 || !panel->aod_mode) {
		...
		rc = dsi_ai2205_tx_cmd_set(panel, DSI_CMD_SET_AOD_LOW);
		panel->aod_mode = 0;
	} else if(panel->panel_last_backlight == 61 || panel->panel_last_backlight == 89 || panel->aod_mode) {
...
static bool dsi_ai2205_validate_c2_last(u32 c2_last)
{
...
bool ai2205_need_skip_data(u32 c2_last)
{
	...
	rc = dsi_ai2205_validate_c2_last(c2_last);
	DSI_LOG("igc=%s\n", rc?"skip":"apply");
	return rc;
}
...
static void dsi_ai2205_dc_bl_set(void)
{
...
bool ai2205_need_backlight_update(void) {
	DSI_LOG("ChargerMode= %d, DCModed = %d", g_Charger_mode, dc_mode_opened);
	return g_Charger_mode || !dc_mode_opened;
}
