...
int dsi_host_transfer_sub(struct mipi_dsi_host *host, struct dsi_cmd_desc *cmd)
{
	...
	if (cmd->ctrl_flags & DSI_CTRL_CMD_BROADCAST) {
		...
		/* ASUS_BSP_DISP +++ */
		cmd->ctrl_flags |= dsi_ai2205_support_cmd_read_flags(cmd->msg.flags);
		...
		void *data)
{
	...
	/* ASUS_BSP_DISP +++ */
	dsi_ai2205_display_init(display);
	goto error;
...
error:
	...
	/* ASUS_BSP_DISP +++ */
	dsi_ai2205_set_panel_is_on(true);
...
int dsi_display_enable(struct dsi_display *display)
{
	...
	if (is_skip_op_required(display)) {
		...
		// TT#367246, workaround for boot animation flicker
		dsi_panel_set_backlight(display->panel,0);
...
int dsi_display_disable(struct dsi_display *display)
{
	...
	/* ASUS_BSP_DISP +++ */
	dsi_ai2205_set_panel_is_on(false);
