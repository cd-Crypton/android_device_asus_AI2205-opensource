...
static irqreturn_t err_fg_interrupt(int irq, void *dev_id)
{
	...
	if ((u32)ktime_to_us(err_ktime1) - (u32)ktime_to_us(err_ktime0) > 60000) {
		ai2205_set_err_fg_irq_state(true);
	}
				...
				enum dsi_cmd_set_type type)
{
	...
	if (!panel->post_on && type != DSI_CMD_SET_PPS) {
...
/* ASUS_BSP_DISP +++ */
int dsi_panel_asus_post_enable(struct dsi_panel *panel)
{
	int rc = 0;
	int diff_ktime;
	...
	diff_ktime = 61000 - ((u32)ktime_to_us(ktime_get()) - (u32)ktime_to_us(init_ktime0));

	// panel init delay time compensation to 61 ms by spec
	if (diff_ktime > 0) {
		diff_ktime = min(61000, diff_ktime);
		DSI_LOG("panel init delay time compensation: %d us", diff_ktime);
		usleep_range(diff_ktime, diff_ktime + 100);
	}
	...
	u32 bl_lvl)
{
	...
	/* ASUS_BSP_DISP +++ */
	if (!panel->post_on) {
		DSI_LOG("post enable before first backlight setting");
		panel->post_on = true;
		rc = dsi_panel_asus_post_enable(panel);
	}

	DSI_LOG("[from] set bl=%d\n", bl_lvl);
	dsi_ai2205_record_backlight(bl_lvl);
	...
	dsi_ai2205_set_dimming_smooth(panel, bl_lvl);
	bl_lvl = dsi_ai2205_backlightupdate(bl_lvl);
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	/* ASUS_BSP_DISP +++ refer to panel dtsi */
