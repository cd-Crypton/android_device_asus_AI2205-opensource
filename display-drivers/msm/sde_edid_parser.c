...
sde_edid_parse_hdr_db(struct drm_connector *connector, const u8 *db)
{
	...
	printk("[DP] HDR electro-optical %d\n", c_conn->hdr_eotf);
	printk("[DP] metadata desc 1 %d\n", c_conn->hdr_metadata_type_one);
	printk("[DP] max luminance %d\n", c_conn->hdr_max_luminance);
	printk("[DP] avg luminance %d\n", c_conn->hdr_avg_luminance);
	printk("[DP] min luminance %d\n", c_conn->hdr_min_luminance);
}
	...
	struct edid *edid)
{
	...
	u32 prod_code = ((u32)edid->prod_code[1] << 4) +
			(edid->prod_code[0] >> 4);
	printk("[DP] sde_edid_parse_extended_blk_info for edid info 0x%x\n", prod_code);
	if (prod_code == 0x770) {
		printk("[DP] exiting parsing extended blk info\n");
		return;
	}
	...
	if (cea && sde_cea_revision(cea) >= 3) {
		...
		sde_for_each_cea_db(cea, i, start, end) {
			...
			if (sde_cea_db_tag(db) == USE_EXTENDED_TAG) {
				printk("[DP] found ext tag block = %d\n",
						db[1]);
				switch (db[1]) {
