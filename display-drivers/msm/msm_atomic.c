...
static void _msm_drm_commit_work_cb(struct kthread_work *work)
{
	...
	if (pm_dev && commit_wakelock) pm_stay_awake(pm_dev);
	power_state_commit_work = power_callback_state;

	// ASUS BSP Display +++
	commit_for_fod_spot = ai2205_atomic_get_spot_status(0);
	report_fod_spot_disappear = ai2205_atomic_get_spot_status(1);
	// ASUS BSP Display ---
	...
	if (report_fod_spot_disappear)
		ai2205_atomic_set_spot_status(1);

	if (commit_for_fod_spot)
		ai2205_atomic_set_spot_status(0);
	// ASUS BSP Display ---

	if(pm_dev && commit_wakelock) pm_relax(pm_dev);
}
