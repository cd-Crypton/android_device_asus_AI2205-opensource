...
int sde_plane_wait_input_fence(struct drm_plane *plane, uint32_t wait_ms)
{
	...
	//pr_err("[Display]:sde_plane_wait_input_fence(power_callback_state=%d)",power_callback_state);
	power_state_when_commit = power_callback_state;
	...
	if (!plane) {
		...
		if (input_fence) {
			...
			{
				int wait_count = wait_ms / 20;
				int count = wait_count;
				rc = 0;
				while(rc == 0 && count >= 0) { // timeout might be 0
					rc = sde_sync_wait(input_fence, 20);
					count--;
					if (power_callback_state != POWER_CALLBACK_STATE_OUT_RESUME) {
						pr_err("[Display]:sde_plane_wait_input_fence skip timeout during suspend/resume.");
						break;
					}
				}
				//pr_err("[Display]:sde_plane_wait_input_fence(%d/%d)(power_callback_state=%d)",
				//	count, wait_count,
				//	power_callback_state);
			}
...
void sde_plane_flush(struct drm_plane *plane)
{
	 ...
	 */
	if (psde->is_error) {
		pr_err("[Display] skip _sde_plane_color_fill() in shipping image!(power:%d,work:%d,commit:%d",
			power_callback_state, power_state_commit_work, power_state_when_commit);
