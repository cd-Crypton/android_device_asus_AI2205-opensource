		...
		struct drm_crtc_state *old_state)
{
	...
	/* ASUS_BSP_DISP +++ */
	//ai2205_crtc_display_commit(crtc);
	ai2205_set_notify_spot_ready(crtc);
	...
	list_for_each_entry(encoder, &dev->mode_config.encoder_list, head) {
		...
		/* ASUS_BSP_DISP +++ */
		ai2205_crtc_display_commit(encoder, crtc);
		ai2205_set_dc_bl_process(encoder, crtc);
	...
	/* ASUS_BSP_DISP +++ */
	dsi_ai2205_frame_commit_cnt(crtc);
		...
		uint64_t val)
{
	...
	case CRTC_PROP_FOD_SPOT:
		...
		ai2205_crtc_fod_masker_spot(crtc, idx, val);
		break;
	/* ASUS_BSP_DISP --- */
	default:
		/* nothing to do */
