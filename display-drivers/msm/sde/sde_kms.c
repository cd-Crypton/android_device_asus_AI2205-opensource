...
bool in_sde_kms_pm_suspend = false; //ASUS_BSP: fix suspend white screen
static int sde_kms_pm_suspend(struct device *dev)
{
	...
	power_callback_state = POWER_CALLBACK_STATE_IN_SUSPEND;
	//DSI_LOG("[Display]:power_callback_state = %d",power_callback_state);
	...
	drm_for_each_encoder(enc, ddev) {
		if (sde_encoder_in_cont_splash(enc) && enc->crtc) {
			...
			power_callback_state = POWER_CALLBACK_STATE_OUT_SUSPEND;
			//DSI_LOG("[Display]:power_callback_state = %d",power_callback_state);
			return -EINVAL;
		}
	}
...
static int sde_kms_pm_resume(struct device *dev)
{
	...
	power_callback_state = POWER_CALLBACK_STATE_IN_RESUME;
	//DSI_LOG("[Display]:power_callback_state = %d",power_callback_state);
	...
	drm_for_each_encoder(enc, ddev) {
		if (sde_encoder_in_cont_splash(enc) && enc->crtc) {
			...
			SDE_EVT32(DRMID(enc), SDE_EVTLOG_FUNC_EXIT);
			power_callback_state = POWER_CALLBACK_STATE_OUT_RESUME;
			//DSI_LOG("[Display]:power_callback_state = %d",power_callback_state);
			return -EINVAL;
		}
	}
