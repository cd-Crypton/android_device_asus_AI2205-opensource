		...
		u32 mode_edid_bpp, u32 mode_pclk_khz, bool dsc_en)
{
	...
	// ASUS BSP Display +++
	// fix TT#407204, set bpp to 24 for monitor TUF Gaming VG28UQL1A (cannot output 2K@120Hz with 30bpp)
	// fix TT#407599, using g_ADAPTER_ID for Ally dock instead of using specific monitor
	if (g_ADAPTER_ID && (mode_pclk_khz * bpp >= 14400000)) {
		DP_LOG("exceed HDMI 2.0 limitation, downgrade bpp to 24");
		bpp = 24;
	}
	// ASUS BSP Display ---
...
static bool dp_panel_read_mst_cap(struct dp_panel *dp_panel)
{
	...
	if (dp_panel->asus_proc_codes == 0x326 && !strncmp(dp_panel->asus_vendor, "AUS", 3)) {
		pr_err("[DP] This panel has issue in MST, disable MST\n");
		mst_cap = false;
	}
