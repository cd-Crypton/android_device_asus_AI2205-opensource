...
/* ASUS_BSP For AMP power down working in system suspend +++ */
#include <linux/ktime.h>
static struct wakeup_source *power_down_wake_lock;
/* ASUS_BSP --- */

#define DRV_NAME "cs35l45"

/* ASUS_BSP For ASUS BSP Customize receive high level command setting feature +++ */
enum asus_control_command {
	ASUS_CONTROL_NONE = 0,
	ASUS_CONTROL_LL_DONOT_MUTE_SETTING  = 2,
	ASUS_CONTROL_MIXER_PATH_VOICE_PCM_SETTING  = 3,
	ASUS_CONTROL_MIXER_PATH_NORMAL_PCM_SETTING  = 4,
	ASUS_CONTROL_RCV_VOICE_PCM_SETTING  = 673,
	ASUS_CONTROL_RCV_NORMAL_PCM_SETTING  = 817,
	ASUS_CONTROL_EE_BYPASS_SETTING  = 5,
};
#define CS35L45_ASUS_CONTROL_RCV_VOICE_PCM_SETTING			0x101770
#define CS35L45_ASUS_CONTROL_RCV_NORMAL_PCM_SETTING			0x101000
/* ASUS_BSP --- */

/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
#define CS35L45_ASUS_SPEAKER_MUSIC_MODE				0X00
#define CS35L45_ASUS_SPEAKER_HEAVY_MODE				0X01
#define CS35L45_ASUS_SPEAKER_OUTDOOR_MODE			0X02
#define CS35L45_ASUS_SPEAKER_VOICE_MODE				0X03

#define CS35L45_ASUS_SPEAKER_INTERIM_MODE			0X04

#define CS35L45_ASUS_SPEAKER_MUSIC_2P1_MODE			0X05
#define CS35L45_ASUS_SPEAKER_HEAVY_2P1_MODE			0X06
#define CS35L45_ASUS_SPEAKER_OUTDOOR_2P1_MODE		0X07
#define CS35L45_ASUS_SPEAKER_VOICE_2P1_MODE			0X08


#define CS35L45_ASUS_RECEIVER_MUSIC_MODE				0X00
#define CS35L45_ASUS_RECEIVER_HEAVY_MODE				0X01
#define CS35L45_ASUS_RECEIVER_OUTDOOR_MODE			0X02
#define CS35L45_ASUS_RECEIVER_VOICE_MODE				0X03

#define CS35L45_ASUS_RECEIVER_INTERIM_MODE			0X04

#define CS35L45_ASUS_RECEIVER_MUSIC_2P1_MODE			0X05
#define CS35L45_ASUS_RECEIVER_HEAVY_2P1_MODE		0X06
#define CS35L45_ASUS_RECEIVER_OUTDOOR_2P1_MODE		0X07
#define CS35L45_ASUS_RECEIVER_VOICE_2P1_MODE			0X08


#define CS35L45_ASUS_WOOFER_MUSIC_MODE				0X00
#define CS35L45_ASUS_WOOFER_HEAVY_MODE				0X01
#define CS35L45_ASUS_WOOFER_OUTDOOR_MODE			0X02
/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/
...
static int cs35l45_supported_devid(struct cs35l45_private *cs35l45)
{
	...
	ret = regmap_read(cs35l45->regmap, CS35L45_BLOCK_ENABLES2, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Block Enable failed\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Block Enable 0x%02X.\n", Current_threshold);
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A --- */
...
/* ASUS_BSP For ASUS Mute service feature +++ */
static void cs35l45_asus_mute_service_finish_mute_delay_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l45_private *cs35l45 =
		container_of(dwork, struct cs35l45_private, asus_mute_service_finish_mute_delay_work);

	dev_err(cs35l45->dev, "cs35l45_asus_mute_service_finish_mute_delay_work : asus_force_mute_status : %d, do_pcm_control : %d\n",
							cs35l45->asus_force_mute_status, cs35l45->do_pcm_control);

	dev_err(cs35l45->dev, "cs35l45_asus_mute_service_finish_mute_delay_work : Set CS35L45_AMP_PCM_CONTROL : 0x%X\n", cs35l45->pcm_control_value);
	regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, cs35l45->pcm_control_value);
	cs35l45->do_pcm_control = false;
	cs35l45->asus_force_mute_status = false;

}
/* ASUS_BSP For ASUS Mute service feature --- */

/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue +++  */
static void cs35l45_ll_do_not_mute_delay_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l45_private *cs35l45 =
		container_of(dwork, struct cs35l45_private, ll_do_not_mute_delay_work);

	dev_err(cs35l45->dev, "%s : Finish LL make do not mute 1 sec(%d)\n", __func__, cs35l45->is_ll_do_not_mute);
	cs35l45->is_ll_do_not_mute = false;
}

static void cs35l45_pmu_finish_mute_delay_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l45_private *cs35l45 =
		container_of(dwork, struct cs35l45_private, pmu_finish_mute_delay_work);

	if (cs35l45->do_flush_pcm_control)	{
		dev_err(cs35l45->dev, "cs35l45_pmu_finish_mute_delay_work : Flush pmu mute do nothing\n");
		cs35l45->do_flush_pcm_control = false;
	} else	{
		dev_err(cs35l45->dev, "cs35l45_pmu_finish_mute_delay_work : asus_force_mute_status : %d, do_pcm_control : %d\n",
							cs35l45->asus_force_mute_status, cs35l45->do_pcm_control);
		if (!cs35l45->asus_force_mute_status)	{
			dev_err(cs35l45->dev, "cs35l45_pmu_finish_mute_delay_work : Set CS35L45_AMP_PCM_CONTROL : 0x%X\n", cs35l45->pcm_control_value);
			regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, cs35l45->pcm_control_value);
			cs35l45->do_pcm_control = false;
		}
	}
...
static void cs35l45_dsp_pmd_delay_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l45_private *cs35l45 =
		container_of(dwork, struct cs35l45_private, dsp_pmd_delay_work);

	__be32 state;
	int i;

	mutex_lock(&cs35l45->dsp_power_lock);
	if (cs35l45->amp_power_status)
		dev_err(cs35l45->dev, "cs35l45_dsp_pmd_delay_work : AMP in power up state do nothing\n");
	else	{
		/*cs35l45->pcm_control_value = cs35l45->pcm_control_value & 0xFFFFF800;
		regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, cs35l45->pcm_control_value);
		dev_err(cs35l45->dev, "cs35l45_dsp_pmd_delay_work begin and reset PCM VOLUME back to : 0x%X\n", cs35l45->pcm_control_value);*/
		dev_err(cs35l45->dev, "cs35l45_dsp_pmd_delay_work begin( PCM VOLUME:0x%X)\n", cs35l45->pcm_control_value);
...
static void cs35l45_global_pmd_delay_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct cs35l45_private *cs35l45 =
		container_of(dwork, struct cs35l45_private, global_pmd_delay_work);

	unsigned int val;

	if (cs35l45->amp_power_status)
		dev_err(cs35l45->dev, "cs35l45_global_pmd_delay_work : AMP in power up state do nothing\n");
	else	{
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	unsigned int pcm_control_value = 0;
	unsigned int pcm_check_control_value = 0;
	unsigned int pcm_mute_value = 0;
	unsigned int sts = 0;
	dev_err(cs35l45->dev, "%s: event = 0x%x, current amp_power_status = %d, ASUS inbox insert status : %d\n", __func__, event, cs35l45->amp_power_status, cs35l45->asus_inbox_insert_status);

			/* Check Volume value +++ */
			ret = regmap_read(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, &pcm_control_value);
			if (ret < 0)
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev : Get CS35L45_AMP_PCM_CONTROL failed\n");
			else
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev : Get CS35L45_AMP_PCM_CONTROL : 0x%X\n", pcm_control_value);

			pcm_check_control_value = pcm_control_value & 0x000007FF;
			if (pcm_check_control_value == 0x4CF)
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev : CS35L45_AMP_PCM_CONTROL is Mute setting do nothing (0x%X)\n", cs35l45->pcm_control_value);
			else
				cs35l45->pcm_control_value = pcm_control_value;
			/* Check Volume value --- */

	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
	if (cs35l45->i2c_addr == 0x32 && event == SND_SOC_DAPM_POST_PMU)	{
		if (cs35l45->asus_inbox_insert_status)	{
			dev_err(cs35l45->dev, "%s ASUS inbox is in insert status, begin do Woofer AMP power up process : %d\n", __func__, event, cs35l45->amp_power_status, cs35l45->asus_inbox_insert_status);
		} else	{
			dev_err(cs35l45->dev, "%s ASUS inbox is NOT in insert status, Abandone Woofer AMP power up process : %d\n", __func__, event, cs35l45->amp_power_status, cs35l45->asus_inbox_insert_status);
			return 0;
		}
	}
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue +++  */
		if (cs35l45->amp_power_status)
			dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : AMP already power up(%d)\n", cs35l45->amp_power_status);
		else	{
			cs35l45->amp_power_status = true;

			if (!cs35l45->is_ll_do_not_mute)	{
				pcm_mute_value = pcm_control_value & 0xFFFFF800;
				pcm_mute_value = pcm_mute_value | 0x4CF;
				regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, pcm_mute_value);
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Set CS35L45_AMP_PCM_CONTROL mute : 0x%X\n", pcm_mute_value);
			} else
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Still in LL do not mute\n");

			dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Abandone  AMP Power down work\n");
			flush_delayed_work(&cs35l45->dsp_pmd_delay_work);
			flush_delayed_work(&cs35l45->global_pmd_delay_work);

			if (!cs35l45->do_pcm_control)	{
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Begin finish mute 200ms work\n");
				cs35l45->do_pcm_control = true;
				queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(200));
			} else	{
				cs35l45->do_flush_pcm_control = true;
				flush_delayed_work(&cs35l45->pmu_finish_mute_delay_work);
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Finish mute work still countdown extend mute time 230ms\n");
				queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(230));
			}

			regmap_read(cs35l45->regmap, CS35L45_DSP_MBOX_2, &sts);

			if (sts == CSPL_MBOX_STS_RUNNING)
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x2 : Get CSPL_MBOX_STS_RUNNING : 0x%X AMP already power up do nothing\n", sts);
			else
			//flush_work(&cs35l45->dsp_pmd_work);
			/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue ---  */
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue +++  */
		if (!cs35l45->amp_power_status)
			dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x4 : AMP already power down(%d)\n", cs35l45->amp_power_status);
		else	{
			cs35l45->amp_power_status = false;

			if (!cs35l45->is_ll_do_not_mute)	{
				pcm_mute_value = pcm_control_value & 0xFFFFF800;
				pcm_mute_value = pcm_mute_value | 0x4CF;
				regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, pcm_mute_value);
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x4 : Set CS35L45_AMP_PCM_CONTROL mute : 0x%X\n", pcm_mute_value);
			} else
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x4 : Still in LL do not mute\n");

			if (!cs35l45->do_pcm_control)	{
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x4 : Begin finish mute 230ms work\n");
				cs35l45->do_pcm_control = true;
				queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(230));
			} else	{
				cs35l45->do_flush_pcm_control = true;
				flush_delayed_work(&cs35l45->pmu_finish_mute_delay_work);
				dev_err(cs35l45->dev, "cs35l45_dsp_power_ev 0x4 : Finish mute work still countdown extend mute time 230ms\n");
				queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(230));
			}
			...
			/* ASUS_BSP For AMP power down working in system suspend */
			__pm_wakeup_event(power_down_wake_lock, 1500);

			queue_delayed_work(cs35l45->wq, &cs35l45->dsp_pmd_delay_work, msecs_to_jiffies(1000));
			/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue ---  */
		}
		break;
	default:
				...
				struct snd_kcontrol *kcontrol, int event)
{
	...
	dev_dbg(cs35l45->dev, "%s: event = 0x%x, ASUS inbox insert status : %d\n", __func__, event, cs35l45->asus_inbox_insert_status);

	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
	if (cs35l45->i2c_addr == 0x32 && event == SND_SOC_DAPM_POST_PMU)	{
		if (cs35l45->asus_inbox_insert_status)	{
			dev_err(cs35l45->dev, "%s ASUS inbox is in insert status, begin do Woofer AMP global enable process : %d\n", __func__, event, cs35l45->amp_power_status, cs35l45->asus_inbox_insert_status);
		} else	{
			dev_err(cs35l45->dev, "%s ASUS inbox is NOT in insert status, Abandone Woofer AMP global enable process : %d\n", __func__, event, cs35l45->amp_power_status, cs35l45->asus_inbox_insert_status);
			return 0;
		}
	}
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/
	...
	case SND_SOC_DAPM_PRE_PMD:
		/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue +++  */
		queue_delayed_work(cs35l45->wq, &cs35l45->global_pmd_delay_work, msecs_to_jiffies(1000));
		/* regmap_read(cs35l45->regmap, CS35L45_BLOCK_ENABLES, &val);
...
static const struct snd_soc_dapm_widget cs35l45_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SWITCH("BBPE Enable", CS35L45_BLOCK_ENABLES2, 13, 1,
			    &bbpe_en_ctl),													//ASUS_BSP For ASUS BSP Customize BBPE Enable issue
...
/* ASUS_BSP For ASUS BSP Customize receive high level command setting feature +++ */
static const char * const asus_pcm_volume_command_texts[] = {"Off","On","LL","673", "817","bypass"};
static SOC_ENUM_SINGLE_DECL(asus_pcm_volume_command_enum, SND_SOC_NOPM, 0,
			    asus_pcm_volume_command_texts);
/* ASUS_BSP --- */


/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
static const char * const asus_outdoor_mode_status_texts[] = {"Off","On"};
static SOC_ENUM_SINGLE_DECL(asus_outdoor_mode_status_enum, SND_SOC_NOPM, 0,
			    asus_outdoor_mode_status_texts);

static const char * const asus_inbox_insert_status_texts[] = {"Off","On"};
static SOC_ENUM_SINGLE_DECL(asus_inbox_insert_status_enum, SND_SOC_NOPM, 0,
			    asus_inbox_insert_status_texts);
/* ASUS_BSP --- */

/* ASUS_BSP For ASUS Mute service feature +++ */
static const char * const asus_mute_service_texts[] = {"Off","On"};
static SOC_ENUM_SINGLE_DECL(asus_mute_service_enum, SND_SOC_NOPM, 0,
			    asus_mute_service_texts);
/* ASUS_BSP --- */

/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
static const char * const asus_csc_cali_status_texts[] = {"Off","On"};
static SOC_ENUM_SINGLE_DECL(asus_csc_cali_status_enum, SND_SOC_NOPM, 0,
			    asus_csc_cali_status_texts);
/* ASUS_BSP --- */
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	struct cs35l45_private *cs35l45 =
			snd_soc_component_get_drvdata(component);
	int ASUS_AMP_Control_Command = 0;
	unsigned int reg_read_value = 0;

	ASUS_AMP_Control_Command = (int)(ucontrol->value.integer.value[0]);

	dev_err(cs35l45->dev, "%s : Get ASUS control command : %d\n", __func__, ASUS_AMP_Control_Command);

	switch (ASUS_AMP_Control_Command) {
		/* For LL don't mute and Flush mute 230ms process +++ */
		case ASUS_CONTROL_LL_DONOT_MUTE_SETTING:
			dev_err(cs35l45->dev, "%s : Is LL make do not mute 1 sec( is LL PCM%d)\n", __func__, cs35l45->is_ll_do_not_mute);
			cs35l45->is_ll_do_not_mute = true;
			cs35l45->do_flush_pcm_control = false;
			flush_delayed_work(&cs35l45->pmu_finish_mute_delay_work);
			queue_delayed_work(cs35l45->wq, &cs35l45->ll_do_not_mute_delay_work, msecs_to_jiffies(500));

			break;
		/* For LL don't mute and Flush mute 230ms process --- */

		/* For RCV voice Digital PCM Volume setting sometime not applied issue +++ */
		case ASUS_CONTROL_MIXER_PATH_VOICE_PCM_SETTING:
		case ASUS_CONTROL_RCV_VOICE_PCM_SETTING:
			cs35l45->pcm_control_value = CS35L45_ASUS_CONTROL_RCV_VOICE_PCM_SETTING;
			regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, cs35l45->pcm_control_value);
			dev_err(cs35l45->dev, "%s : Set CS35L45_AMP_PCM_CONTROL : (%d) 0x%X\n", __func__, ASUS_AMP_Control_Command, cs35l45->pcm_control_value);
			
			break;
		case ASUS_CONTROL_MIXER_PATH_NORMAL_PCM_SETTING:
		case ASUS_CONTROL_RCV_NORMAL_PCM_SETTING:
			cs35l45->pcm_control_value = CS35L45_ASUS_CONTROL_RCV_NORMAL_PCM_SETTING;
			regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, cs35l45->pcm_control_value);
			dev_err(cs35l45->dev, "%s : Set CS35L45_AMP_PCM_CONTROL : (%d) 0x%X\n", __func__, ASUS_AMP_Control_Command, cs35l45->pcm_control_value);
		...
		/* For Audio EE bypass  issue +++ */
		case ASUS_CONTROL_EE_BYPASS_SETTING:
			regmap_read(cs35l45->regmap, CS35L45_BLOCK_ENABLES2, &reg_read_value);
			dev_dbg(cs35l45->dev, "%s: get current CS35L45_GLOBAL_ENABLES : 0x%x \n", __func__, reg_read_value);
			regmap_read(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, &reg_read_value);
			dev_dbg(cs35l45->dev, "%s: get current CS35L45_BST_BPE_IL_LIM_THLD : 0x%x \n", __func__, reg_read_value);

			regmap_write(cs35l45->regmap, CS35L45_BLOCK_ENABLES2, 0X08000110);
			regmap_write(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, 0X00000032);

			usleep_range(100, 200);

			regmap_read(cs35l45->regmap, CS35L45_BLOCK_ENABLES2, &reg_read_value);
			dev_dbg(cs35l45->dev, "%s: get after setting CS35L45_GLOBAL_ENABLES : 0x%x \n", __func__, reg_read_value);
			regmap_read(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, &reg_read_value);
			dev_dbg(cs35l45->dev, "%s: get after setting CS35L45_BST_BPE_IL_LIM_THLD : 0x%x \n", __func__, reg_read_value);

			break;
		/* For Audio EE bypass  issue --- */

		default:
			dev_err(cs35l45->dev, "%s : Invalid command = 0x%x\n", __func__, ASUS_AMP_Control_Command);
	}
	/* ASUS_BSP For ASUS BSP Customize receive high level command setting --- */
	return 0;
}


/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
static int cs35l45_asus_outdoor_mode_status_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct cs35l45_private *cs35l45 =
			snd_soc_component_get_drvdata(component);
	int ASUS_Inbox_Insert_Status = 0;
	int ASUS_Outdoor_Mode_Status = 0;

	ASUS_Inbox_Insert_Status = cs35l45->asus_inbox_insert_status;
	ASUS_Outdoor_Mode_Status = cs35l45->asus_outdoor_mode_status;

	dev_dbg(cs35l45->dev, "%s : Get ASUS Outdoor Mode : %d (ASUS inbox insert status : %d)\n", __func__, ASUS_Outdoor_Mode_Status, ASUS_Inbox_Insert_Status);

	ucontrol->value.integer.value[0] = ASUS_Outdoor_Mode_Status;

	return 0;
}

static int cs35l45_asus_outdoor_mode_status_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ASUS_Outdoor_Mode_Status = (int)(ucontrol->value.integer.value[0]);
	cs35l45->asus_outdoor_mode_status = (bool)ASUS_Outdoor_Mode_Status;

	ASUS_Inbox_Insert_Status = cs35l45->asus_inbox_insert_status;

	dev_err(cs35l45->dev, "%s : Set ASUS Outdoor Mode : %d (ASUS inbox insert status : %d)\n", __func__, ASUS_Outdoor_Mode_Status, ASUS_Inbox_Insert_Status);
	return 0;
}

static int cs35l45_asus_inbox_insert_status_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	dev_dbg(cs35l45->dev, "%s : Get ASUS inbox insert status : %d (ASUS Outdoor Mode : %d)\n", __func__, ASUS_Inbox_Insert_Status, ASUS_Outdoor_Mode_Status);

	ucontrol->value.integer.value[0] = ASUS_Inbox_Insert_Status;

	return 0;
}

static int cs35l45_asus_inbox_insert_status_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ASUS_Inbox_Insert_Status = (int)(ucontrol->value.integer.value[0]);
	cs35l45->asus_inbox_insert_status = (bool)ASUS_Inbox_Insert_Status;

	ASUS_Outdoor_Mode_Status = cs35l45->asus_outdoor_mode_status;

	dev_err(cs35l45->dev, "%s : Set ASUS inbox insert status : %d (ASUS Outdoor Mode : %d)\n", __func__, ASUS_Inbox_Insert_Status, ASUS_Outdoor_Mode_Status);
...
/* ASUS_BSP For ASUS Mute service feature +++ */
static int cs35l45_asus_mute_service_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	int ASUS_Inbox_Insert_Status = 0;
	int ASUS_Outdoor_Mode_Status = 0;
	int ASUS_mute_service_Status = 0;

	ASUS_Inbox_Insert_Status = cs35l45->asus_inbox_insert_status;
	ASUS_Outdoor_Mode_Status = cs35l45->asus_outdoor_mode_status;
	ASUS_mute_service_Status = cs35l45->asus_force_mute_status;

	dev_dbg(cs35l45->dev, "%s : Get ASUS mute servicce status : %d (ASUS inbox insert status : %d, ASUS Outdoor Mode : %d)\n", __func__
								, ASUS_mute_service_Status, ASUS_Inbox_Insert_Status, ASUS_Outdoor_Mode_Status);

	ucontrol->value.integer.value[0] = ASUS_mute_service_Status;

	return 0;
}

static int cs35l45_asus_mute_service_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ASUS_mute_service_Status = (int)(ucontrol->value.integer.value[0]);

	ASUS_Inbox_Insert_Status = cs35l45->asus_inbox_insert_status;
	ASUS_Outdoor_Mode_Status = cs35l45->asus_outdoor_mode_status;

	dev_err(cs35l45->dev, "%s : Get ASUS mute servicce status : %d (ASUS inbox insert status : %d, ASUS Outdoor Mode : %d)\n", __func__
								, ASUS_mute_service_Status, ASUS_Inbox_Insert_Status, ASUS_Outdoor_Mode_Status);

	/* Check Volume value +++ */
	ret = regmap_read(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, &pcm_control_value);
	if (ret < 0)
		dev_err(cs35l45->dev, "%s : Get CS35L45_AMP_PCM_CONTROL failed\n", __func__);
	else
		dev_err(cs35l45->dev, "%s : Get CS35L45_AMP_PCM_CONTROL : 0x%X\n", __func__, pcm_control_value);

	pcm_check_control_value = pcm_control_value & 0x000007FF;
	if (pcm_check_control_value == 0x4CF)
		dev_dbg(cs35l45->dev, "%s : CS35L45_AMP_PCM_CONTROL is Mute setting do nothing (0x%X)\n", __func__, cs35l45->pcm_control_value);
	else
		cs35l45->pcm_control_value = pcm_control_value;
	/* Check Volume value --- */


	if (ASUS_mute_service_Status)	{
		if (ASUS_Inbox_Insert_Status)	{
			cs35l45->asus_force_mute_status = true;
			pcm_mute_value = pcm_control_value & 0xFFFFF800;
			pcm_mute_value = pcm_mute_value | 0x4CF;
			regmap_write(cs35l45->regmap, CS35L45_AMP_PCM_CONTROL, pcm_mute_value);
			dev_err(cs35l45->dev, "%s : Set CS35L45_AMP_PCM_CONTROL mute : 0x%X\n", __func__, pcm_mute_value);

			cs35l45->do_pcm_control = true;
			/* Maybe need TODO extent mute time */
			queue_delayed_work(cs35l45->wq, &cs35l45->asus_mute_service_finish_mute_delay_work, msecs_to_jiffies(3000));			
			dev_err(cs35l45->dev, "%s : Get ASUS mute servicce status = %d and inbox in insert state, Begin Mute  AMP )\n", __func__
								, ASUS_mute_service_Status);
		} else	{
			dev_dbg(cs35l45->dev, "%s : Get ASUS mute servicce status = %d but inbox NOT in insert state, Do nothing)\n", __func__
								, ASUS_mute_service_Status);
			cs35l45->asus_force_mute_status = false;
		}
	} else	{
		dev_dbg(cs35l45->dev, "%s : Get ASUS mute servicce status = %d(ASUS inbox insert status : %d, ASUS Outdoor Mode : %d, Do nothing)\n", __func__
								, ASUS_mute_service_Status, ASUS_Inbox_Insert_Status, ASUS_Outdoor_Mode_Status);
		cs35l45->asus_force_mute_status = false;
	}
...
/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
static int cs35l45_asus_csc_cali_status_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	struct cs35l45_private *cs35l45 =
			snd_soc_component_get_drvdata(component);
	int ASUS_csc_cali_status = 0;

	ASUS_csc_cali_status = cs35l45->asus_csc_cali_status;

	dev_dbg(cs35l45->dev, "%s : Get ASUS CSC AMP Calibration status : %d \n", __func__ , ASUS_csc_cali_status);

	ucontrol->value.integer.value[0] = ASUS_csc_cali_status;

	return 0;
}

static int cs35l45_asus_csc_cali_status_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
			snd_soc_kcontrol_component(kcontrol);
	...
	ASUS_csc_cali_status = (int)(ucontrol->value.integer.value[0]);


	dev_err(cs35l45->dev, "%s : Set ASUS CSC AMP Calibration status Command: %d ( Old ASUS CSC AMP Calibration status )\n", __func__ , ASUS_csc_cali_status, cs35l45->asus_csc_cali_status);

	cs35l45->asus_csc_cali_status = ASUS_csc_cali_status;
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A, Woofer set 0x3c 4A +++ */
	ret = regmap_read(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Level 1 Current threshold failed\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Level 1 Current threshold 0x%02X.( 0x%X )\n", Current_threshold, cs35l45->i2c_addr);
	Current_threshold = 0;

	if (cs35l45->i2c_addr == 0x32)
		regmap_write(cs35l45->regmap, CS35L45_BST_BPE_IL_LIM_THLD, 0x3C);
	else
	...
	regmap_update_bits(cs35l45->regmap,
				   CS35L45_BLOCK_ENABLES2,
				   CS35L45_BBPE_EN_MASK, 1<<CS35L45_BBPE_EN_SHIFT);

	/* Enable ASP
	regmap_update_bits(cs35l45->regmap,
				   CS35L45_BLOCK_ENABLES2,
				   CS35L45_ASP_EN_MASK, 1<<CS35L45_ASP_EN_SHIFT);*/

	ret = regmap_read(cs35l45->regmap, CS35L45_BLOCK_ENABLES2, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Block Enable failed again\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Reset Block Enable 0x%02X.\n", Current_threshold);
	/* ASUS_BSP Set Level 1 Current threshold from default 0x2C 3A to 0x36 3.625A --- */



	/* ASUS_BSP Set Woofer Class H algorithm Controls HDRM VDD_BST headroom to 0.5v +++ */
	ret = regmap_read(cs35l45->regmap, CS35L45_CLASSH_CONFIG1, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Woofer CS35L45_CLASSH_CONFIG1 failed\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: CS35L45_CLASSH_CONFIG1 0x%02X.( 0x%X )\n", Current_threshold, cs35l45->i2c_addr);
	Current_threshold = 0;

	if (cs35l45->i2c_addr == 0x32)
		regmap_update_bits(cs35l45->regmap,
				   CS35L45_CLASSH_CONFIG1,
				   CS35L45_CH_HDRM_MASK, 10 << CS35L45_CH_HDRM_SHIFT);

	ret = regmap_read(cs35l45->regmap, CS35L45_CLASSH_CONFIG1, &Current_threshold);
	if (ret < 0)
		dev_err(cs35l45->dev, "Get Woofer CS35L45_CLASSH_CONFIG1 failed again\n");
	else
		dev_err(cs35l45->dev, "Cirrus Logic CS35L45: Reset CS35L45_CLASSH_CONFIG1 0x%02X.( 0x%X )\n", Current_threshold, cs35l45->i2c_addr);
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
	if (cs35l45->asus_csc_cali_status)	{
		dev_err(cs35l45->dev, "%s : Get device 0x%X in CSC AMP Calibration status, do NOT do fast switch \n", __func__, cs35l45->i2c_addr);
		return ret;
	}
	/* ASUS_BSP For ASUS CSC AMP Calibration feature --- */
					...
					struct snd_ctl_elem_value *ucontrol)
{
	...
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++ */
	dev_err(cs35l45->dev, "%s :	AMP Get setting fast switch mode : %d ( 0x%X )\n",__func__ ,i ,cs35l45->i2c_addr);

	if (cs35l45->i2c_addr == 0x30)	{
		/* Receiver AMP operation mode process */
		if (cs35l45->asus_inbox_insert_status)	{

			switch (i) {
				case CS35L45_ASUS_RECEIVER_MUSIC_MODE:
					if (cs35l45->asus_outdoor_mode_status)	{
						cs35l45->fast_switch_file_idx = CS35L45_ASUS_RECEIVER_OUTDOOR_2P1_MODE;
						dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_RECEIVER_OUTDOOR_2P1_MODE : %d ( 0x%X )\n"
									,__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					} else	{
						cs35l45->fast_switch_file_idx = CS35L45_ASUS_RECEIVER_MUSIC_2P1_MODE;
						dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_RECEIVER_MUSIC_2P1_MODE : %d ( 0x%X )\n",
									__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					}


					break;
				case CS35L45_ASUS_RECEIVER_OUTDOOR_MODE:
					cs35l45->fast_switch_file_idx = CS35L45_ASUS_RECEIVER_OUTDOOR_2P1_MODE;
					dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_RECEIVER_OUTDOOR_2P1_MODE : %d ( 0x%X )\n",
								__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					break;
				default:
					dev_err(cs35l45->dev, "%s :	 Normal %d( 0x%X )\n",__func__,i , cs35l45->i2c_addr);
			...
			cs35l45->fast_switch_file_idx = i;
	} else if (cs35l45->i2c_addr == 0x31)	{
		/* Speaker AMP operation mode process */
		if (cs35l45->asus_inbox_insert_status)	{
			switch (i) {
				case CS35L45_ASUS_SPEAKER_MUSIC_MODE:
					if (cs35l45->asus_outdoor_mode_status)	{
						cs35l45->fast_switch_file_idx = CS35L45_ASUS_SPEAKER_OUTDOOR_2P1_MODE;
						dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_SPEAKER_OUTDOOR_2P1_MODE : %d ( 0x%X )\n",
									__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					} else	{
						cs35l45->fast_switch_file_idx = CS35L45_ASUS_SPEAKER_MUSIC_2P1_MODE;
						dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_SPEAKER_MUSIC_2P1_MODE : %d ( 0x%X )\n",
									__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					}
					break;
				case CS35L45_ASUS_SPEAKER_OUTDOOR_MODE:
					cs35l45->fast_switch_file_idx = CS35L45_ASUS_SPEAKER_OUTDOOR_2P1_MODE;
					dev_err(cs35l45->dev, "%s :	In Inbox insert state switch to 2p1 CS35L45_ASUS_SPEAKER_OUTDOOR_2P1_MODE : %d ( 0x%X )\n",
								__func__ ,cs35l45->fast_switch_file_idx ,cs35l45->i2c_addr);
					break;
	...
	/* Woofer AMP operation mode process */
	} else if (cs35l45->i2c_addr == 0x32)	{
		if (cs35l45->asus_outdoor_mode_status)	{
			cs35l45->fast_switch_file_idx = CS35L45_ASUS_WOOFER_OUTDOOR_MODE;
		} else
			cs35l45->fast_switch_file_idx = i;		
					 ...
					 struct snd_ctl_elem_value *ucontrol)
{
	...
	unsigned int pcm_check_control_value = 0;
	unsigned int pcm_mute_value = 0;
	dev_err(cs35l45->dev, "%s: After Flush : begin mute AMP (do_PCM_control = %d)\n", __func__, cs35l45->do_pcm_control);
	...
	pcm_check_control_value = pcm_control_value & 0x000007FF;
	if (pcm_check_control_value == 0x4CF)
		dev_err(cs35l45->dev, "%s : CS35L45_AMP_PCM_CONTROL is Mute setting do nothing (0x%X)\n", __func__, cs35l45->pcm_control_value);
	else
		cs35l45->pcm_control_value = pcm_control_value;
	/* Check Volume value --- */
	...
	if (!cs35l45->is_ll_do_not_mute)	{
		...
		if (!cs35l45->do_pcm_control)	{
			dev_err(cs35l45->dev, "%s : Begin finish mute 230ms work\n", __func__);
			cs35l45->do_pcm_control = true;
			queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(230));
		} else	{
			cs35l45->do_flush_pcm_control = true;
			flush_delayed_work(&cs35l45->pmu_finish_mute_delay_work);
			dev_err(cs35l45->dev, "%s  : Finish mute work still countdown extend mute time 230ms\n", __func__);
			queue_delayed_work(cs35l45->wq, &cs35l45->pmu_finish_mute_delay_work, msecs_to_jiffies(230));
		}
	} else
		dev_err(cs35l45->dev, "%s : Still in LL do not mute (do_PCM_control = %d)\n", __func__, cs35l45->do_pcm_control);
	/* For LL don't mute and Flush mute 230ms process --- */
...
#endif
	/* ASUS_BSP For ASUS BSP Customize receive high level command setting feature */
	SOC_ENUM_EXT("AMP Active Status", asus_pcm_volume_command_enum,
			   cs35l45_amp_active_status_get, cs35l45_amp_active_status_put),


	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
	SOC_ENUM_EXT("Outdoor Mode Status", asus_outdoor_mode_status_enum,
			   cs35l45_asus_outdoor_mode_status_get, cs35l45_asus_outdoor_mode_status_put),

	SOC_ENUM_EXT("Inbox Insert Status", asus_inbox_insert_status_enum,
			   cs35l45_asus_inbox_insert_status_get, cs35l45_asus_inbox_insert_status_put),
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/

	/* ASUS_BSP For ASUS Mute service feature +++ */
	SOC_ENUM_EXT("ASUS Mute Service", asus_mute_service_enum,
			   cs35l45_asus_mute_service_get, cs35l45_asus_mute_service_put),
	/* ASUS_BSP For ASUS Mute service feature --- */

	/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
	SOC_ENUM_EXT("ASUS CSC Cali Status", asus_csc_cali_status_enum,
			   cs35l45_asus_csc_cali_status_get, cs35l45_asus_csc_cali_status_put),
	/* ASUS_BSP For ASUS CSC AMP Calibration feature --- */
...
int cs35l45_initialize(struct cs35l45_private *cs35l45)
{
	...
	unsigned int dev_id = 0, rev_id = 0;
	...
	ret = __cs35l45_initialize(cs35l45);
	if (ret < 0) {
		dev_err(dev, "CS35L45 failed to initialize force to Audio system continue (%d)\n", ret);
		cs35l45->initialized = false;
		//return ret;
	}
	...
	if (cs35l45->irq && cs35l45->initialized) {
		...
		dev_err(dev, "force to Audio system continue without request IRQ(%d)\n", ret);

	dev_err(dev, "Cirrus Logic CS35L45 (%x), Revision: %02X\n", dev_id,
		 rev_id);
...
int cs35l45_probe(struct cs35l45_private *cs35l45)
{
	...
	/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue +++  */
	INIT_DELAYED_WORK(&cs35l45->dsp_pmd_delay_work, cs35l45_dsp_pmd_delay_work);
	INIT_DELAYED_WORK(&cs35l45->global_pmd_delay_work, cs35l45_global_pmd_delay_work);
	INIT_DELAYED_WORK(&cs35l45->pmu_finish_mute_delay_work, cs35l45_pmu_finish_mute_delay_work);
	INIT_DELAYED_WORK(&cs35l45->ll_do_not_mute_delay_work, cs35l45_ll_do_not_mute_delay_work);
	cs35l45->amp_power_status = false;
	cs35l45->do_pcm_control = false;
	cs35l45->do_flush_pcm_control = false;
	cs35l45->is_ll_do_not_mute = false;
	cs35l45->pcm_control_value = 0;
	/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue ---  */

	/* ASUS_BSP Register wake_lock for AMP power down working in system suspend */
	power_down_wake_lock = wakeup_source_register(cs35l45->dev, "cs35l45_power_down_lock");

	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
	cs35l45->asus_outdoor_mode_status = false;
	cs35l45->asus_inbox_insert_status = false;
	cs35l45->asus_force_mute_status = false;
	INIT_DELAYED_WORK(&cs35l45->asus_mute_service_finish_mute_delay_work, cs35l45_asus_mute_service_finish_mute_delay_work);
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/

	/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
	cs35l45->asus_csc_cali_status = false;
	/* ASUS_BSP For ASUS CSC AMP Calibration feature --- */
