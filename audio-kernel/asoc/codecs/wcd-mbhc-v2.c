...
/* ASUS_BSP +++ Fix TT415014:headset hook key sometimes can't play or pause music (system suspend) */
...
static int wcd_mbhc_initialise(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_cfg->enable_usbc_analog) {
		...
		WCD_MBHC_REG_UPDATE_BITS(WCD_MBHC_INSREM_DBNC, 0xC); /* ASUS_BSP change to 768ms for ASUS ROG6 HW design */
	}
		      ...
		      bool impedance_det_en)
{
    ...
    /* ASUS_BSP +++ Fix TT415014:headset hook key sometimes can't play or pause music (system suspend) */
	hook_key_wake_lock = wakeup_source_register(card->dev, "wcd_hook_key_lock");
