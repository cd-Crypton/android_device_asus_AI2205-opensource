...
struct cs35l45_private {
	...
	struct delayed_work dsp_pmd_delay_work;
	struct delayed_work global_pmd_delay_work;
	struct delayed_work pmu_finish_mute_delay_work;
	struct delayed_work ll_do_not_mute_delay_work;	
	bool amp_power_status;
	bool do_pcm_control;
	bool do_flush_pcm_control;
	bool is_ll_do_not_mute;	
	unsigned int pcm_control_value;
	/* ASUS_BSP Add AMP power down debounce feature for switch speaker pop sound issue ---  */

	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature +++*/
	bool asus_outdoor_mode_status;
	bool asus_inbox_insert_status;
	bool asus_force_mute_status;
	struct delayed_work asus_mute_service_finish_mute_delay_work;
	/* ASUS_BSP For Rog7 Inbox 2p1 channel feature ---*/

	/* ASUS_BSP For ASUS CSC AMP Calibration feature +++ */
	bool asus_csc_cali_status;
	/* ASUS_BSP For ASUS CSC AMP Calibration feature --- */
