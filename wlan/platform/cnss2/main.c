		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	res = kstrtoint(buf,10,&antenna_gpio);

	key_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR_OR_NULL(key_pinctrl)) {
	    pr_err("[cnss] get_pinctrl failed and res: %d", res);
	}
...
/* ASUS_BSP--- "add for the antenna switch power (LDO13A)" */
	...
	wlan_asus_ant_gpio = of_get_named_gpio(dev->of_node,"wlan-asus_ant_160",0);
	...
	if (gpio_is_valid (wlan_asus_ant_gpio)) {
		ret = gpio_request(wlan_asus_ant_gpio, "wlan-asus_ant_160");
