...
public class A2dpService extends ProfileService {
    ...
    private long mLastSetActiveTime = 0;
    private static final int DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE = 0;
    private static final int DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE_TIME = 1000;
    private BluetoothDevice lastActiveDevice;
    ...
    private Handler mActiveDeviceHandler = new Handler() {
        @Override
       public void handleMessage(Message msg) {
           Log.v(TAG, "processMessage: received message=" + msg.what);
           switch (msg.what) {
               case DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE:
    ...
    private boolean setActiveDeviceInternal(BluetoothDevice device) {
        ...
        if (device == null) {
            ...
            //If the time from active to null is too short, delay sending removeActiveDevice
            if (previousActiveDevice.equals(lastActiveDevice) && mAdapterService.isTwsPlusDevice(previousActiveDevice)) {
                long now = System.currentTimeMillis();
                long lapse = now - mLastSetActiveTime;
                mLastSetActiveTime = 0;
                Log.d(TAG, "time lapse since last set active device: " + lapse);
                if (lapse < DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE_TIME) {
                    Log.d(TAG, "lapse is too short, sleep: " + (DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE_TIME - lapse));
                    Message msg = mActiveDeviceHandler.obtainMessage(DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE);
                    mActiveDeviceHandler.sendMessageDelayed(msg, DELAY_NOTIFY_ACTIVE_DEVICE_REMOVE_TIME - lapse);
                } else {
                    removeActiveDevice(false);
                }
            } else {
        ...
        if (device != null && mAdapterService.isTwsPlusDevice(device)) {
            lastActiveDevice = device;
            mLastSetActiveTime = System.currentTimeMillis();
            Log.d(TAG, "mLastSetActiveTime: " + mLastSetActiveTime);
        };
            ...
            switch((int)(cs4 & APTX_MODE_MASK)) {
                ...
                case APTX_LL:
                  if (mActivityManager != null) {
                      ...
                      if(currentApp != null && currentApp.equals("com.microsoft.teams")) {
                          Log.i(TAG, "skip teams in setting APTX_LL");
                          return;
                      }
                  }
