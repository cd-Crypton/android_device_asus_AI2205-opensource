...
class BluetoothPbapUtils {
    ...
    static void loadAllContacts(Context context, Handler handler, PbapStateMachine sm)
        ...
        sTotalContacts = fetchAndSetContacts(context, handler, projection, null, null, true, sm);
        if (sTotalContacts < 0) {
            sTotalContacts = 0;
            return;
        }
        handler.sendMessage(handler.obtainMessage(BluetoothPbapService.CONTACTS_LOADED, sm));
    }
    ...
    private static int fetchAndSetContacts(Context context, Handler handler, String[] projection,
            String whereClause, String[] selectionArgs, boolean isLoad, PbapStateMachine sm) {
        ...
        if (c == null) {
            ...
            if (isLoad) {
                handler.sendMessageDelayed(
                        handler.obtainMessage(BluetoothPbapService.LOAD_CONTACTS, sm),
