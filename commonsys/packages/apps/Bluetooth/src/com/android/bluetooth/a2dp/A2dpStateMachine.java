...
final class A2dpStateMachine extends StateMachine {
    ...
    private int mLastBroadcastConnState = BluetoothProfile.STATE_DISCONNECTED;
    ...
    public void doQuit() {
        ...
        if (getConnectionState() != BluetoothProfile.STATE_DISCONNECTED) {
            broadcastConnectionState(BluetoothProfile.STATE_DISCONNECTED, mLastBroadcastConnState);
        }
        quitNow();
    }
            ...
            switch (event) {
                ...
                case A2dpStackEvent.CONNECTION_STATE_CONNECTING:
                    if (mA2dpService.okToConnect(mDevice, false)) {
                        ...
                        AsusEvtLog.writeAsusEvtLog(TAG,
                                "A2DP accept incoming request: " + mDevice);
                        transitionTo(mConnecting);
    ...
    private void broadcastConnectionState(int newState, int prevState) {
        ...
        AsusEvtLog.writeAsusEvtLog(TAG, "A2DP connection state " + profileStateToString(prevState)
            + "->" + profileStateToString(newState) + ": "+ mDevice);
        ...
        mLastBroadcastConnState = newState;
