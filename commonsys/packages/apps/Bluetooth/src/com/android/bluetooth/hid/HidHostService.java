...
public class HidHostService extends ProfileService {
    ...
    private static final int MESSAGE_GP_CONNECT = 17;

    // Asus dongle detect
    private int dock_state = 0;
    private int GamePad_num = 0;
    private static final String AsusGamePadNameList[] = {
        "ROG Kunai Gamepad",
        "ROG Kunai 3 Gamepad",
    };

    private static final String AsusRogPhoneNameList[] = {
        "ZS661KS", //ROG3
        "ZS673KS", //ROG5
        "ZS676KS", //ROG5.5
        "AI2201", //ROG6
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                break;
                case MESSAGE_GP_CONNECT: {
                    BluetoothDevice device = (BluetoothDevice) msg.obj;
                    updateGPstate(true, device);
                }
                break;
            }
        }
...
@Override
    ...
    private void broadcastConnectionState(BluetoothDevice device, int newState) {
        ...
        if(isROG()){
            if(isGamePad(device) && newState == BluetoothProfile.STATE_CONNECTED){
                //wait 100ms for update of VID/PID info
                Message msg = mHandler.obtainMessage(MESSAGE_GP_CONNECT);
                msg.obj = device;
                mHandler.sendMessageDelayed(msg, 100);
            } else if(isGamePad(device) && newState == BluetoothProfile.STATE_DISCONNECTED && prevState != BluetoothProfile.STATE_CONNECTING){
            if(mHandler.hasMessages(MESSAGE_GP_CONNECT, device)){
                mHandler.removeMessages(MESSAGE_GP_CONNECT, device);
                Log.e(TAG, "No need updateGPstate for disconnection  becuase no perevious updateGPstate for connection");
                return;
            }
            updateGPstate(false, device);
            }
        }
    ...
    private boolean isGamePad(BluetoothDevice device) {
        String remoteName = Utils.getName(device);

        for(int i = 0; i < AsusGamePadNameList.length; i++){
            String name = AsusGamePadNameList[i];
            if(remoteName != null && remoteName.equals(name)) {
                Log.d(TAG, remoteName + ", isGamePad = true");
                return true;
            }
        }
        return false;
    ...
    private boolean isROG() {
        String asusProduct = SystemProperties.get("ro.build.product");

        for(int i = 0; i < AsusRogPhoneNameList.length; i++){
            String name = AsusRogPhoneNameList[i];
            if(asusProduct != null && asusProduct.equals(name))
                return true;
        }
        return false;
    }

    private void updateGPstate(boolean connected, BluetoothDevice device) {
        dock_state = SystemProperties.getInt("sys.asus.donglestate_GV_PD",0);
        GamePad_num = SystemProperties.getInt("sys.asus.GamePad_num",0);
        Log.d(TAG, "current dock_state=" + dock_state + " ,GamePad_num=" + GamePad_num);

        Intent intent = new Intent(Intent.ACTION_DOCK_EVENT);
        intent.addFlags(Intent.FLAG_RECEIVER_REPLACE_PENDING);

        if(connected){
            GamePad_num = GamePad_num  + 1;
            if(dock_state == 0 || dock_state == 1){
                //EXTRA_DOCK_STATE_ASUS_GAMEVICE = 14
                intent.putExtra(Intent.EXTRA_DOCK_STATE, 14);
                dock_state = 1;
            } else if(dock_state == 2 || dock_state == 3){
                //EXTRA_DOCK_STATE_PD_GV = 15
                intent.putExtra(Intent.EXTRA_DOCK_STATE, 15);
                dock_state = 3;
            } else {
                //not expected dock_state, set to current state
                Log.e(TAG, "wrong EXTRA_DOCK_STATE " + dock_state + ", EXTRA_DOCK_STATE set EXTRA_DOCK_STATE_ASUS_GAMEVICE");
                //EXTRA_DOCK_STATE_ASUS_GAMEVICE = 14
                intent.putExtra(Intent.EXTRA_DOCK_STATE, 14);
                dock_state = 1;
            }
            ...
            if(dock_state == 3){
                if (GamePad_num <= 0 ){
                   ...
                   intent.putExtra(Intent.EXTRA_DOCK_STATE, 13);
                   dock_state = 2;
                } else {
                    //EXTRA_DOCK_STATE_PD_GV = 15
                    intent.putExtra(Intent.EXTRA_DOCK_STATE, 15);
                ...
                if (GamePad_num <= 0 ){
                    intent.putExtra(Intent.EXTRA_DOCK_STATE, 0);
                    dock_state = 0;
                 } else {
                    //EXTRA_DOCK_STATE_ASUS_GAMEVICE = 14
                    intent.putExtra(Intent.EXTRA_DOCK_STATE, 14);
                ...
                //not expected dock_state, set to 0
                Log.e(TAG, "wrong EXTRA_DOCK_STATE "+ dock_state +", EXTRA_DOCK_STATE set 0");
                intent.putExtra(Intent.EXTRA_DOCK_STATE, 0);
                dock_state=0;
            }
         }
         ...
         if(GamePad_num < 0) {
            Log.e(TAG, "wrong GamePad_num "+ GamePad_num +", GamePad_num set 0");
            GamePad_num = 0;
         }
         Log.d(TAG, "set dock_state=" + dock_state + " ,GamePad_num=" + GamePad_num);
         SystemProperties.set("sys.asus.GamePad_num", Integer.toString(GamePad_num));
         SystemProperties.set("sys.asus.donglestate_GV_PD", Integer.toString(dock_state));
