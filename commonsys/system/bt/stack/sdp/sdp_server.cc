          ...
          if (!strncmp("false", a2dp_role, 5)) {
            ...
            if (ver >= AVRC_REV_1_4) {
              ...
              SDP_TRACE_DEBUG("%s :Showing AVRCP version in SDP = 0x%x", __func__,
                               p_attr->value_ptr[PROFILE_VERSION_POSITION]);
              } else if (!strncmp("true", pts_property, 4)) {
                p_attr->value_ptr[PROFILE_VERSION_POSITION] = (uint8_t)(AVRC_REV_1_6 & 0x00ff);
                SDP_TRACE_DEBUG("%s: runnung pts now, return avrcp 1.6", __func__);
            } else {
