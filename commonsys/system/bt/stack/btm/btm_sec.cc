...
void btm_sec_connected(const RawAddress& bda, uint16_t handle, uint8_t status,
  ...
  if (p_dev_rec) {
    VLOG(2) << __func__ << ": Security Manager: in state: "
            ...
            << " RName:" << p_dev_rec->sec_bd_name;
    if(status == HCI_SUCCESS) {
       snprintf(cmds, sizeof(cmds), "[BT] Connected to %s RName: %s\n", bda.ToString().c_str(), p_dev_rec->sec_bd_name);
    } else {
       snprintf(cmds, sizeof(cmds), "[BT] Fail to Connect to %s RName: %s, reason:0x%02x\n", bda.ToString().c_str(), p_dev_rec->sec_bd_name, status);
    }
     WriteASUSEvtLog(cmds);
  } else {
    VLOG(2) << __func__ << ": Security Manager: in state: "
            << btm_pair_state_descr(btm_cb.pairing_state)
            << " handle:" << handle << " status:" << loghex(status)
            << " enc_mode:" << loghex(enc_mode) << " bda:" << bda;
    if(status == HCI_SUCCESS) {
        snprintf(cmds, sizeof(cmds), "[BT] Connected to %s \n", bda.ToString().c_str());
    } else {
        snprintf(cmds, sizeof(cmds), "[BT] Fail to Connect to %s, reason:0x%02x\n", bda.ToString().c_str(),  status);
    }
  }
...
/*******************************************************************************
 *
 * Function         btm_sec_is_drop_reason_expected
 *
 * Description      This function is to check the drop connection is expected or not
 *
 * Returns          bool
 *
 ******************************************************************************/

bool btm_sec_is_drop_reason_expected (uint8_t reason) {
    if (reason == HCI_SUCCESS || reason == HCI_ERR_PAGE_TIMEOUT ||
        reason == HCI_ERR_CONNECTION_TOUT || reason == HCI_ERR_PEER_USER ||
        reason == HCI_ERR_PEER_POWER_OFF || reason == HCI_ERR_CONN_CAUSE_LOCAL_HOST || reason == HCI_ERR_CONN_FAILED_ESTABLISHMENT){
...
void btm_sec_disconnected(uint16_t handle, uint8_t reason) {
  ...
  char cmds[1024];
  snprintf(cmds, sizeof(cmds), "[BT] Disconnected from %s reason: 0x%02x RName: %s\n", p_dev_rec->bd_addr.ToString().c_str(), reason, p_dev_rec->sec_bd_name);
  WriteASUSEvtLog(cmds);
  if (!btm_sec_is_drop_reason_expected(reason)) {
    LOG_WARN(LOG_TAG, "%s BT disconnected with Unexpected reason: %d", __func__, reason);
	char Unexpected [1024];
	snprintf(Unexpected, sizeof(Unexpected), "[BT] Unexpected disconnection from %s reason: 0x%02x RName: %s\n", p_dev_rec->bd_addr.ToString().c_str(), reason, p_dev_rec->sec_bd_name);
    WriteASUSEvtLog(Unexpected);
  }
