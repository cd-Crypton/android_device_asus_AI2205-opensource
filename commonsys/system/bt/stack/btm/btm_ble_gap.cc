...
void btm_ble_periodic_adv_sync_lost(uint8_t *param, uint16_t param_len) {
  ...
  int index = btm_ble_get_psync_index_from_handle(sync_handle);
  if (index == MAX_SYNC_TRANSACTION) {
    BTM_TRACE_ERROR("[PSync]: index not found for handle %u",__func__ , sync_handle);
    return;
  }
  tBTM_BLE_PERIODIC_SYNC *ps = &btm_ble_pa_sync_cb.p_sync[index];
...
void btm_send_hci_scan_enable(uint8_t enable, uint8_t filter_duplicates) {
  if (controller_get_interface()->supports_ble_extended_advertising()) {
    BTM_TRACE_WARNING("%s ble_set_extended_scan_enable: %x", __func__, enable);
    btsnd_hcic_ble_set_extended_scan_enable(enable, filter_duplicates, 0x0000,
                                            0x0000);
  } else {
    BTM_TRACE_WARNING("%s ble_set_scan_enable: %x", __func__, enable);
