  ...
  if (p_data != NULL && status == GATT_SUCCESS) {
    ...
    switch (read_type) {
      ...
      case GATT_UUID_IEEE_DATA:
        ...
        if(read_type == GATT_UUID_FW_VERSION_STR &&
          p_clcb->dis_value.pnp_id.vendor_id == 0x0b05 &&
          p_clcb->dis_value.pnp_id.product_id == 0x7906){
          LOG(INFO)  << "current gamepad3 bt fw version:  " << p_str;
          osi_property_set("vendor.remotefw.gamepad3.bt_fwver", (char*)p_str);
        }
        break;
...
bool DIS_ReadDISInfo(const RawAddress& peer_bda, tDIS_READ_CBACK* p_cback,
  ...
  if (p_cback == NULL && mask != DIS_ATTR_FW_NUM_BIT) return (false);
