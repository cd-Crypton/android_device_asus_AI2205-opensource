...
void bta_hh_le_dis_cback(const RawAddress& addr, tDIS_VALUE* p_dis_value) {
  ...
  if (p_dis_value->attr_mask & DIS_ATTR_PNP_ID_BIT) {
    ...
    if(p_dis_value->pnp_id.vendor_id == 0x0b05 && p_dis_value->pnp_id.product_id == 0x7906){
      APPL_TRACE_WARNING("read FW NUM for gamepad3");
      if (!DIS_ReadDISInfo(p_cb->addr, NULL, DIS_ATTR_FW_NUM_BIT)) {
...
void bta_hh_security_cmpl(tBTA_HH_DEV_CB* p_cb,
  ...
  if (p_cb->status == BTA_HH_OK) {
    ...
    if(p_cb->dscp_info.vendor_id == 0x0b05 && p_cb->dscp_info.product_id == 0x7906){
        APPL_TRACE_WARNING("%s: Starting service discovery for checking gamepad3 bt fw", __func__);
        bta_hh_le_pri_service_discovery(p_cb);
    }

    else if (p_cb->app_id != 0 && p_cb->hid_srvc[0].in_use) {
...
void bta_hh_gatt_close(tBTA_HH_DEV_CB* p_cb, tBTA_HH_DATA* p_data) {
  ...
  if(p_cb->dscp_info.vendor_id == 0x0b05 && p_cb->dscp_info.product_id == 0x7906){
    APPL_TRACE_WARNING("gamepad3 disconnect, set gamepad3 bt fw version to 0");
    osi_property_set("vendor.remotefw.gamepad3.bt_fwver", "0");
  }
