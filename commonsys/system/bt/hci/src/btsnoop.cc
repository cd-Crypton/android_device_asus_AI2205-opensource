...
#define LOG_COLLECTION_DIR "/data/misc/bluetooth/logs/"
#define ASUS_BTSNOOP_LOG_STATUS "debug.bluetooth.btsnoop_status"
#define MAX_BTSNOOP_COUNT 10
...
static void updateBtsnoopMode();
static void delete_snoop_if_required();
...
static std::string get_btsnoop_log_path();
...
static future_t* start_up() {
  ...
  if (is_btsnoop_enabled() <= 0) {
    delete_btsnoop_files();
  } else {
    LOG_DEBUG(LOG_TAG,"start up btsnoop");
    delete_snoop_if_required();
    updateBtsnoopMode();
...
static void capture(const BT_HDR* buffer, bool is_received) {
  ...
  //For record log without bt on/off
  if (is_btsnoop_enabled() > 0) {
      if (logfile_fd == INVALID_FD || is_btsnoop_enabled() == BTSNOOP_ASUS_START) {
          LOG_DEBUG(LOG_TAG, "%s open_next_snoop_file after the BT is in Enable state", __func__ );
          open_next_snoop_file();
          packets_per_file = osi_property_get_int32(BTSNOOP_MAX_PACKETS_PROPERTY,
                                              DEFAULT_BTSNOOP_SIZE);
      } else if (is_btsnoop_enabled() == BTSNOOP_ASUS_LOGGING && logfile_fd != INVALID_FD) {
          //// Re-check btsnoop file status
          auto log_path = get_btsnoop_log_path();
          if(stat(log_path.c_str(), &st) != 0 && errno == ENOENT) {
              LOG_DEBUG(LOG_TAG, "%s btsnoop file dose not exist re-create it", __func__ );
              open_next_snoop_file();
          }
      }
  } else {
...
static void delete_btsnoop_files() {
  LOG(INFO) << __func__ << ": Deleting snoop logs if they exist.";

  //persist.asus.startlog is true when DUT install UTS log tool/ASUS log tool(enable logcat service)
  char asus_logging[PROPERTY_VALUE_MAX] = {0};
  osi_property_get("persist.vendor.asus.startlog", asus_logging, "0");
  if (strncmp(asus_logging, "1", 1) == 0  ){
    LOG_DEBUG(LOG_TAG, "%s asus logcat service still working not to delet btsnoop", __func__);
    return;
  }

  struct DIR* p_dir;
  struct dirent* p_dirent;
  char oldest_log_path[PROPERTY_VALUE_MAX + sizeof(".mm_dd_hh_mm_ss") + sizeof(LOG_COLLECTION_DIR)];
  ...
  while ((p_dirent = readdir(p_dir)) != NULL) {
    if (strstr(p_dirent->d_name, "btsnoop_hci")) {
      snprintf(oldest_log_path,PROPERTY_VALUE_MAX + sizeof(".mm_dd_hh_mm_ss") + sizeof(LOG_COLLECTION_DIR),"%s%s", LOG_COLLECTION_DIR, p_dirent->d_name);
      if(remove(oldest_log_path) < 0)
        LOG_DEBUG(LOG_TAG, "Cannot delete file: %s", oldest_log_path);
    }
  }
}
...
static int is_btsnoop_enabled() {
  ...
  char asus_btsnoop_status[PROPERTY_VALUE_MAX] = {0};
  char btsnoop_status[PROPERTY_VALUE_MAX] = {0};
  osi_property_get(BTSNOOP_LOG_MODE_PROPERTY, btsnoop_status, "-1");
  osi_property_get(ASUS_BTSNOOP_LOG_STATUS, asus_btsnoop_status, "-1");
  //if DUT reboot when btsnoop is logging, keep logging
  if ((strncmp(btsnoop_status, "full", 4) == 0) && (strncmp(asus_btsnoop_status, "-1", 2) == 0)) {
      osi_property_set(ASUS_BTSNOOP_LOG_STATUS, "start");
      updateBtsnoopMode();
      return BTSNOOP_ASUS_START;
  }

  if (strncmp(asus_btsnoop_status, "start", 5) == 0) {   // start new file
      updateBtsnoopMode();
      return BTSNOOP_ASUS_START;
  } else if (strncmp(asus_btsnoop_status, "stop", 4) == 0) {   // stop logging
      //updateBtsnoopMode();
      return BTSNOOP_ASUS_STOP;
  } else if (strncmp(asus_btsnoop_status, "logging", 7) == 0 ) {  // log with same file
      return BTSNOOP_ASUS_LOGGING;
  } else {
      return BTSNOOP_ASUS_NA;
  }
}

std::string get_btsnoop_log_path() {
...
std::string get_btsnoop_last_log_path(std::string btsnoop_path) {
  ...
  //record time mm_dd_hh_mm
  snprintf(Nowtime,sizeof(Nowtime),".%02d_%02d_%02d_%02d_%02d",nowtime->tm_mon + 1,nowtime->tm_mday,nowtime->tm_hour,nowtime->tm_min,nowtime->tm_sec);
  std::string result(Nowtime);
  return btsnoop_path.append(result);
}
...
static void delete_snoop_if_required(){
  ...
  int count;
  count = scandir(LOG_COLLECTION_DIR, &entry_list, filter_dot, alphasort);
  LOG_DEBUG(LOG_TAG, "%s btsnoop_number_limit: %d", __func__,  MAX_BTSNOOP_COUNT);

  if(count <= 0){
    LOG_DEBUG(LOG_TAG, "%s Cannot scan dump location or No snoop file", __func__ );
    return;
  }

  if(count > MAX_BTSNOOP_COUNT){
    first_entry = entry_list[1]->d_name;
    snprintf(oldest_log_path,PROPERTY_VALUE_MAX + sizeof(".mm_dd_hh_mm_ss") + sizeof(LOG_COLLECTION_DIR),"%s%s", LOG_COLLECTION_DIR, first_entry);
    LOG_DEBUG(LOG_TAG, "Delete oldest snoop path: %s", oldest_log_path);
    if(remove(oldest_log_path) < 0)
      LOG_DEBUG(LOG_TAG, "Cannot delete file: %s", oldest_log_path);
  }
  free(entry_list);
...
static void open_next_snoop_file() {
  ...
  if(is_btsnoop_enabled() == BTSNOOP_ASUS_START ){
     if (rename(log_path.c_str(), last_log_path.c_str()) != 0 && errno != ENOENT)
      ...
      osi_property_set(ASUS_BTSNOOP_LOG_STATUS, "logging");
  }

  if(stat(log_path.c_str(), &st) == 0) {
    LOG(INFO) << "last path : " << log_path << " already exist";
    isappended = true;
  }

  mode_t prevmask = umask(0);
  logfile_fd = open(log_path.c_str(), O_WRONLY | O_CREAT | O_APPEND,
  ...
  if (isappended == false) {
    LOG(INFO) << __func__ << " Write btsnoop header";
